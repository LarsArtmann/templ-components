...
2. **`utils/svg.FillIcon`** — 20×20 filled icons via `fillIcon` templ component. Uses `fill="currentColor"`. Used for chevrons (accordion, dropdown), trend arrows (stat card), and status indicators. (Was `internal/svg`, promoted to `utils/svg` in ADR-0034 for cross-module access.)
...
- `utils/svg` provides shared SVG path constants and fill-icon rendering — consumers use `icons.Icon` for their own icons.

## Consequences

- Two small SVG systems coexist.
- `utils/svg.FillIcon` is used by display/navigation packages for small directional indicators.
