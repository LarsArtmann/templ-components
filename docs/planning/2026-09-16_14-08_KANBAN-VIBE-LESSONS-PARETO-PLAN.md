...
- **TODO_LIST #191** has asked since 2026-09-09 which KanbanBoard feature-scope props
  ...
  - **within-column keyboard reorder → stays deferred** (vibe-kanban *blocks* within-column reorder outside manual sort — no evidence it's wanted; our prev/next-column buttons remain the keyboard story)
...
- Why next: the only real *functional* gap. A per-column add-card affordance is standard
...
|---|------|------|-----|--------|--------|----------------|--------------------|
| M1 | Write `docs/recipes/kanban-card-anatomy.md` (anatomy, tag-overflow `2+N`, avatar row, description-preview snippet, sorted-views section) + verify snippets compile | 1% | 90min | High | S | High — unlocks vibe's proven card UX with zero API change | Recipe file merged; every Go snippet compiles (example test or demo build); cross-linked from `KanbanCard` godoc |
| M2 | Sorted-views contract notes: `KanbanBoardProps.Wire` godoc + `ParseKanbanMove` godoc (server-side rejection guidance for sorted views) | 1% | 30min | High | S | High — prevents the #1 predictable consumer bug | Godoc on both symbols; wording matches recipe section |
| M3 | Implement `KanbanColumn.Action templ.Component` — per-column add-card slot, full test ladder | 4% | 100min | High | M | High — closes the only real functional gap | Field + render + goldens + a11y assertions + example test + CHANGELOG entry; `nix run .#verify` green |
| M4 | Implement `KanbanColumn.Tone` status dot — `KanbanTone` enum mirroring `StatTone`, map + dark pairs, full test ladder | 20% | 90min | Med | M | Med — instant column semantics, themable via existing model | Enum + `IsValid` + test; dot renders before title; dark-mode compliance green; goldens; CHANGELOG |
| M5 | Demo wiring: `Action` + `Tone` on both kanban demo boards; recompile demo CSS if needed | 80% | 45min | Med | S | Med — demo is the adoption shop window | Demo renders both features under both transports; CSS freshness guard green |
| M6 | Regenerate visual route goldens (light+dark) after demo markup change; review PNG diffs | 80% | 30min | Med | S | Med — CI stays green, visual history honest | `nix run .#visual` green on first run after `-update`; diffs eyeballed |
| M7 | Hygiene: TODO_LIST #191 close-out + #190 evidence note + new IDs; FEATURES.md Kanban entry; CHANGELOG `[Unreleased]` warm | 80% | 30min | Med | S | Med — living docs stay truthful | TODO_LIST updated (decision recorded, next-free-ID bumped); FEATURES + CHANGELOG mention both props |
| M8 | Full verification sweep: `nix run .#verify` + per-module loop + visual suite | 80% | 30min | High | S | High — proof nothing regressed | All green in one sitting; results noted in plan footer |
...
| ID | Task | Est | Gate |
|----|------|-----|------|
| P1.1 | Read `docs/recipes/horizontal-filter-bar.md` + one more recipe for tone/structure conventions | 5min | — |
| P1.2 | Draft recipe skeleton: intro, when-to-use, section list | 10min | skeleton reads like existing recipes |
| P1.3 | Write card-anatomy section: ID `Badge` + title + priority `StatusBadge` example | 12min | snippet compiles |
| P1.4 | Write tag-overflow section: render 2 `Badge`s + `N+` remainder (CountBadge convention) | 12min | snippet compiles |
| P1.5 | Write assignee section: `Avatar` row via `Content` slot | 10min | snippet compiles |
| P1.6 | Write description-preview section: markdown→one-line Go helper snippet (vibe-kanban's placeholder-label trick, adapted) | 12min | snippet compiles |
| P1.7 | Write sorted-views section: when moves are meaningful; server-side rejection pattern in the move handler | 12min | — |
| P1.8 | Add compiling proof: fold snippets into an `example` test or extend `kanban` example test | 12min | `go test ./display/...` green |
| P1.9 | Cross-link: `KanbanCard.Content` + `KanbanBoardProps.Wire` godoc → recipe; recipe → research doc | 8min | godoc mentions recipe path |
| P1.10 | `ParseKanbanMove` godoc: add the sorted-view rejection guidance sentence | 10min | — |

### Phase 2 — 4% → 64%: `KanbanColumn.Action` (M3, ~100min)

| ID | Task | Est | Gate |
|----|------|-----|------|
| P2.1 | Add `Action templ.Component` field + godoc to `KanbanColumn` in `kanban.templ` | 10min | godoc explains slot semantics (rendered in header, consumer owns transport) |
| P2.2 | Render `col.Action` in the column header row, after the count badge (`flex` row already exists) | 12min | read-only boards render it too; nil-safe |
| P2.3 | `templ generate` (pinned dev shell!) + `go build ./display/...` | 8min | zero unrelated diff (templ pin v0.3.1020) |
| P2.4 | Golden sweep: add `Action` variant to `golden_sweep_test.go`; `-update`; eyeball diff | 12min | goldens show slot in header |
| P2.5 | A11y test: slot content lands inside the header `div`, no aria breakage, focus order sane | 12min | a11y test added + green |
| P2.6 | Example test: godoc `ExampleKanbanBoard` variant with an Action button | 10min | green |
| P2.7 | CHANGELOG `[Unreleased]` **Added** entry + FEATURES.md Kanban line | 10min | warm-rule satisfied |
| P2.8 | `nix run .#verify` | 12min | green |

### Phase 3 — 20% → 80%: `KanbanColumn.Tone` (M4, ~90min)

| ID | Task | Est | Gate |
|----|------|-----|------|
| P3.1 | Define `KanbanTone` enum mirroring `StatTone` (`card.templ:245`): gray default + blue/green/yellow/red/purple, consts `KanbanTone*` | 12min | zero value = toneless (no dot) |
| P3.2 | Add `KanbanToneIsValid` + test in the package enum test file (same commit!) | 10min | IsValid-rule satisfied |
| P3.3 | `kanbanToneLookup: map[KanbanTone]string` with complete literal classes incl. `dark:` pairs; render via `utils.Lookup` | 12min | dark-mode compliance pattern followed |
| P3.4 | Render dot (`h-2 w-2 rounded-full`) before column title when tone set; `aria-hidden` decorative | 10min | goldens reflect dot |
| P3.5 | `templ generate` + `go build` | 8min | zero unrelated diff |
| P3.6 | Golden sweep variants per tone; `-update`; eyeball | 12min | — |
| P3.7 | Run `TestDarkModeCompliance` + `TestDarkModeSemanticColors` + full display tests | 5min | green |
| P3.8 | CHANGELOG + FEATURES entries | 8min | — |
| P3.9 | `nix run .#verify` | 12min | green |

### Phase 4 — other 20% → 100%: Demo, goldens, hygiene, sweep (M5–M8, ~135min)

| ID | Task | Est | Gate |
|----|------|-----|------|
| P4.1 | Demo: wire an `Action` add-button into demo board columns (`examples/demo/kanban_demo.templ`) | 12min | renders under both transports |
| P4.2 | Demo: set `Tone` per demo column (todo=blue, doing=yellow, done=green) | 10min | — |
| P4.3 | Recompile demo CSS (`nix run .#css` path per CSS freshness guard) if new classes entered `.templ` sources | 12min | `TestCSSFreshness` green |
| P4.4 | Rebuild + restart demo binary; smoke `/kanban-board` manually | 10min | both features visible |
| P4.5 | Regenerate route goldens light+dark (`visual` suite `-update`) | 12min | — |
| P4.6 | Eyeball PNG diffs; commit updated goldens | 12min | no unexplained pixel shifts |
| P4.7 | Full visual suite green run (no `-update`) | 6min | green |
| P4.8 | TODO_LIST: record #191 decision (add-card IN, tone IN, WIP OUT, reorder stays deferred) pointing at plan + research doc | 8min | row updated |
| P4.9 | TODO_LIST: #190 add drag-handle evidence note (research §4.5: only if whole-card click actions ever land) | 5min | row updated |
| P4.10 | TODO_LIST: add open IDs #218 (Action), #219 (Tone), #220 (recipe) — marked with this plan; bump next-free-ID to 221 | 10min | — |
| P4.11 | Final sweep: per-module test loop (`for mod in utils icons errorpage charts/echarts datastar htmx; …`) + `nix run .#verify` + `nix flake check` | 12min | all green |
| P4.12 | Update this plan's footer with actual results/timestamps | 6min | — |
