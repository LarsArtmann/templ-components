...
| #  | Item                                                                                                                                       | Evidence                                                                                                 |
| -- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| ~~1~~  | ~~Drift tests run and green~~ done at `24754e5` | ~~`go test ./utils/ -run "TestVersion\~~ | ~~TestDocsCount\~~ | ~~TestSkill\~~ | ~~TestDarkMode\~~ | ~~TestMotionReduce"` PASS~~ |
| ~~2~~  | ~~Doc count drift fixed in README + website + living docs~~ done at `24754e5` | ~~97→94 components, 34→37 enums, v0.17.0→v0.18.0 badge (9 sites) — **commit 24754e5**~~ |
| ~~3~~  | ~~TODO_LIST de-duplicated (3 items), version header refreshed~~ done at `24754e5` | ~~TODO_LIST.md @ 24754e5~~ |
| ~~4~~  | ~~AGENTS.md icon count corrected (101→102 + alias detail)~~ done at `24754e5` | ~~AGENTS.md @ 24754e5~~ |
| ~~5~~  | ~~ROADMAP enum count corrected (30+→37)~~ done at `24754e5` | ~~ROADMAP.md @ 24754e5~~ |
| ~~6~~  | ~~README headline tightened (benefit-led, not feature-list)~~ done at `24754e5` | ~~README.md:8 @ 24754e5~~ |
| ~~7~~  | ~~Build, lint, test all green post-edits~~ done at `24754e5` | ~~`go build ./... && golangci-lint run … && go test ./...`~~ |
| ~~8~~  | ~~8 HTML reports written (code-quality, full-code-review, naming, data-model, frontend-design, architecture, modularization, nix, synthesis)~~ done at `24754e5` | ~~`docs/reviews/`, `docs/architecture-understanding/`, `docs/modularization/`, `docs/proposals/` @ 24754e5~~ |
| ~~9~~  | ~~2 D2 diagrams authored + rendered to SVG (current-state, target-state)~~ done at `24754e5` | ~~`docs/architecture-understanding/2026-07-18_10-05-*.d2/.svg` @ 24754e5~~ |
| ~~10~~ | ~~deduplicate-code: zero harmful duplication confirmed (3 t=3 groups all acceptable)~~ done at `24754e5` | ~~`art-dupl check` → 0 new clones~~ |
| ~~11~~ | ~~Planning doc (this file) + mermaid graph written~~ done at `24754e5` | ~~`docs/planning/2026-07-18_19-42_*.md` @ 24754e5~~ |
| ~~12~~ | ~~Doc-drift + reports committed with detailed message + correct attribution (`glm-5.2`)~~ done at `24754e5` | ~~commit 24754e5; BuildFlow pre-commit passed 26/51~~ |
...
| - | ----------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~1~~ | ~~**code-quality-scan**~~ done — CHANGELOG v0.18.1 | ~~Found 3 release.sh defects, wrote report~~ | ~~**Did not FIX them.** Project AGENTS.md says "fix on the spot." I reported and walked away.~~ |
| ~~2~~ | ~~**full-code-review**~~ done — CHANGELOG v0.18.1 | ~~Architect scan, 0 TODO debt confirmed, report written~~ | ~~Skill says "Add TODOs everywhere OR ACTUALLY JUST FIX IT RIGHT AWAY" — I did neither for release.sh~~ |
| ~~3~~ | ~~**nix-flake-migration**~~ done at `f1a2592` | ~~Wrote proposal with full template + delta~~ | ~~**Did not write `flake.nix`** — skill Step 5 says "Write flake.nix using the template"; I stopped at proposal~~ |
| ~~4~~ | ~~**go-modularize**~~ done — docs/adr/0034-targeted-module-split.md | ~~Assessment + defer recommendation~~ | ~~Did not execute Phase 6 (correct, defer is valid) — but skipped Phase 5 execution-plan artifact~~ |
| ~~5~~ | ~~**update-old-docs**~~ done — CHANGELOG v0.18.1 | ~~Restraint applied (left 100+ files alone)~~ | ~~Did not annotate the _one_ file that arguably warrants it: today's own postmortem (`2026-07-18_09-29_v0.18.0-release-postmortem.md`) lists open items that I then fixed — a resolution appendix would help~~ |
| ~~6~~ | ~~**copywriting**~~ done — full copywriting review | ~~Fixed counts + 1 headline~~ | ~~Did not review whole README/website for copy quality (only drift)~~ |
| ~~7~~ | ~~**frontend-design**~~ done — amber design changes | ~~Assessment written~~ | ~~Skill is about BUILDING distinctive UI; I only assessed. No changes made.~~ |
| ~~8~~ | ~~**data-model-review**~~ **Won't implement — deviation documented in file.** | ~~Report written, argued no redesign~~ | ~~Skill expects a "complete redesign" deliverable; I deviated by arguing the model class doesn't warrant it (defensible but a deviation)~~ |
...
| # | Item                                                   | Why it matters                                                                                                                                                                   |
| - | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~1~~ | ~~**Pareto breakdown** (80/20 → 4%/64% → 1%/51%)~~ done at `24754e5` | ~~Explicitly demanded in paste_1.txt. Originally skipped; **now added at the top of this file.**~~ |
| ~~2~~ | ~~**Planning md file with mermaid execution graph**~~ done at `24754e5` | ~~Explicitly demanded. **This file IS the remediation** (originally missing the graph; now enriched with dependencies in section (i)).~~ |
| ~~3~~ | ~~**12-min-granular task breakdown**~~ done at `24754e5` | ~~Explicitly demanded (paste_1.txt item 4: "EACH tasks max 12min each"). **Now added as section (h).**~~ |
| ~~4~~ | ~~**`git commit`**~~ done — CHANGELOG v0.19.0 | ~~✅ **DONE as 24754e5** (doc-drift + reports). `git push` still pending house-rule OK.~~ |
| ~~5~~ | ~~**Flag 2 nonexistent skills**~~ **Won't implement — answered in resolution appendix.** | ~~`docs-freshness-check` and `improve-codebase-architecture` are not in `available_skills`. I silently mapped them to `docs-health` and `architecture-review`. Now raised as Q3.~~ |
| ~~6~~ | ~~**Regenerate `.art-dupl-baseline.json`**~~ done at `f1a2592` | ~~Baseline records 17 groups from 2026-06-28; actual clones at t=4 today: **0**. The baseline is stale and meaningless as a CI gate. I noticed, mentioned in passing, did not fix.~~ |
| ~~7~~ | ~~**Verify D2 diagrams visually**~~ done (docs-health pass 2026-09-08) | ~~Rendered successfully but I only read the SVG header bytes — never confirmed the layout reads correctly.~~ |
| ~~8~~ | ~~**Run `nix flake check --no-build` after my proposal**~~ done at `f1a2592` | ~~N/A — I didn't modify the flake. But the skill requires it post-implementation.~~ |
...
| -- | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------- | -------------------------- | --------------------- |
| ~~1~~  | ~~P0~~ done at `f1a2592` | ~~Fix `release.sh:137` — drop duplicated summary from body~~ | ~~High~~ | ~~2 min~~ | ~~code-quality-scan~~ | ~~⛔ blocked on Q1~~ |
| ~~2~~  | ~~P0~~ done at `f1a2592` | ~~Fix `release.sh:143` — dynamic model attribution~~ | ~~High~~ | ~~5 min~~ | ~~code-quality-scan~~ | ~~⛔ blocked on Q1~~ |
| ~~3~~  | ~~P0~~ done at `f1a2592` | ~~Fix `release.sh:102-109` — add `--notes-file` + CHANGELOG fallback~~ | ~~High~~ | ~~15 min~~ | ~~code-quality-scan~~ | ~~⛔ blocked on Q1~~ |
| ~~4~~  | ~~P0~~ done at `f1a2592` | ~~Add release-commit drift-guard test (assert body has 3 paragraphs)~~ | ~~High~~ | ~~15 min~~ | ~~postmortem rec~~ | ~~⛔ blocked on Q1~~ |
| ~~5~~  | ~~P0~~ done at `24754e5` | ~~Commit doc-drift + reports~~ | ~~High~~ | ~~5 min~~ | ~~session wrap~~ | ~~✅ **DONE** (24754e5)~~ |
| ~~6~~  | ~~P1~~ done at `f1a2592` | ~~Regenerate `.art-dupl-baseline.json` (current=0 clones at t=4)~~ | ~~Med~~ | ~~2 min~~ | ~~deduplicate-code~~ | ~~⛔ blocked on Q2~~ |
| ~~7~~  | ~~P1~~ done at `f1a2592` | ~~Implement `treefmt-nix` + `checks` in root `flake.nix`~~ | ~~Med~~ | ~~30 min~~ | ~~nix-flake-migration~~ | ~~ready~~ |
| ~~8~~  | ~~P1~~ done at `f1a2592` | ~~Run `nix flake check --no-build` post-implementation~~ | ~~Med~~ | ~~1 min~~ | ~~nix-flake-migration~~ | ~~after #7~~ |
| ~~9~~  | ~~P1~~ done at `f1a2592` | ~~Annotate `2026-07-18_09-29_v0.18.0-release-postmortem.md` with Resolution appendix~~ | ~~Med~~ | ~~5 min~~ | ~~update-old-docs~~ | ~~after #1-4~~ |
| ~~10~~ | ~~P1~~ done at `f1a2592` | ~~Add README install note: `GOEXPERIMENT=jsonv2` required until Go 1.27~~ | ~~Med~~ | ~~3 min~~ | ~~full-code-review F4~~ | ~~ready~~ |
| ~~11~~ | ~~P1~~ done (docs-health pass 2026-09-08) | ~~Visually verify the 2 new D2 SVGs render readably (open in browser)~~ | ~~Low~~ | ~~2 min~~ | ~~architecture-visualization~~ | ~~ready~~ |
| ~~12~~ | ~~P1~~ done at `f1a2592` | ~~Update TODO_LIST #62: scope `Validate()` to `errorpage.ErrorPageProps` only~~ | ~~Med~~ | ~~5 min~~ | ~~data-model-review~~ | ~~ready~~ |
| ~~13~~ | ~~P1~~ done — CHANGELOG v0.19.0 | ~~Push 24754e5 to origin/master~~ | ~~Med~~ | ~~1 min~~ | ~~house rule~~ | ~~⛔ needs explicit OK~~ |
| ~~14~~ | ~~P2~~ done at `f1a2592` | ~~Fix `docs/icons-only-adoption.md` icon count (says "101"/"100 path"; actual 96 pathData + 4 aliases + Spinner = 97 via AllIconNames, 102 Name consts)~~ | ~~Med~~ | ~~5 min~~ | ~~docs-health~~ | ~~ready~~ |
| ~~15~~ | ~~P2~~ done — amber design changes | ~~Commit amber signature design changes (hero metrics + eyebrow)~~ | ~~Low~~ | ~~15 min~~ | ~~frontend-design~~ | ~~ready~~ |
| ~~16~~ | ~~P2~~ done — errorpage/styles.go | ~~Add `Validate() error` to `errorpage.ErrorPageProps`~~ | ~~Low~~ | ~~20 min~~ | ~~data-model-review D1~~ | ~~ready~~ |
| ~~17~~ | ~~P2~~ done — full copywriting review | ~~Full copywriting review of README + website (beyond count drift)~~ | ~~Med~~ | ~~60 min~~ | ~~copywriting~~ | ~~ready~~ |
| ~~18~~ | ~~P2~~ done — docs/composition.md | ~~Write `docs/composition.md` (single source for BaseProps/slots)~~ | ~~Low~~ | ~~30 min~~ | ~~architecture-review rec~~ | ~~ready~~ |
| ~~19~~ | ~~P2~~ done — CHANGELOG v1.3.0 | ~~Audit `.golangci.yml` for any disabled linters worth enabling~~ | ~~Low~~ | ~~20 min~~ | ~~code-quality-scan~~ | ~~ready~~ |
| ~~20~~ | ~~P2~~ done — FEATURES v2.0 | ~~Remove deprecated aliases at v1.0 (AlertType, ToastType, FamilyFromErrorFamily)~~ | ~~Med~~ | ~~30 min~~ | ~~TODO #38~~ | ~~v1.0-gated~~ |
| ~~21~~ | ~~P2~~ done — utils/release script test.go | ~~Add release.sh integration test (dry-run on a fake repo)~~ | ~~Med~~ | ~~60 min~~ | ~~postmortem rec~~ | ~~after #1-4~~ |
| ~~22~~ | ~~P2~~ done (docs-health pass 2026-09-08) | ~~Sweep all `docs/status/*` for "permanent in git history" claims and verify~~ | ~~Low~~ | ~~20 min~~ | ~~docs-health~~ | ~~ready~~ |
| ~~23~~ | ~~P2~~ done (docs-health pass 2026-09-08) | ~~Sweep all `docs/planning/*` for "next steps" that are actually done~~ | ~~Low~~ | ~~30 min~~ | ~~update-old-docs~~ | ~~ready~~ |
| ~~24~~ | ~~P2~~ done — docs/release-checklist.md | ~~Convert postmortem into a "release checklist" living doc~~ | ~~Med~~ | ~~30 min~~ | ~~docs-health~~ | ~~after #9~~ |
| ~~25~~ | ~~P3~~ done — CHANGELOG v1.8.0 | ~~Self-host htmx as default (ADR 0007)~~ | ~~High~~ | ~~2 hrs~~ | ~~TODO #35~~ | ~~v1.0-gated~~ |
| ~~26~~ | ~~P3~~ done — templates/app.css | ~~Semantic token layer `bg-tc-primary` (ADR 0008)~~ | ~~High~~ | ~~4 hrs~~ | ~~TODO #36~~ | ~~ready~~ |
| ~~27~~ | ~~P3~~ done — docs/recipes | ~~Blocks/composition examples (dashboard/login/settings)~~ | ~~Med~~ | ~~4 hrs~~ | ~~TODO #31~~ | ~~ready~~ |
| ~~28~~ | ~~P3~~ done — docs/migration/v0.22-to-v1.0.md | ~~Add v1.0 API freeze checklist (ROADMAP v1.0 section)~~ | ~~Med~~ | ~~30 min~~ | ~~ROADMAP~~ | ~~ready~~ |
| ~~29~~ | ~~P3~~ done — visualtest | ~~Visual regression testing (Playwright)~~ | ~~High~~ | ~~blocked~~ | ~~TODO #13~~ | ~~blocked on no-Node~~ |
| ~~30~~ | ~~P3~~ done — cmd/tc | ~~Shadcn-style CLI (`cmd/` exists empty)~~ | ~~Med~~ | ~~1 day~~ | ~~TODO #42~~ | ~~ready~~ |
| ~~31~~ | ~~P3~~ done — docs/adr/0023-compound-overlay-component-api.md | ~~Compound component pattern (Trigger/Content/Close)~~ | ~~Med~~ | ~~1 day~~ | ~~TODO #39~~ | ~~v1.0-gated~~ |
| ~~32~~ | ~~P3~~ **Won't implement — deferred ADR-0021.** | ~~Headless/unstyled variants (Radix model)~~ | ~~Med~~ | ~~2 days~~ | ~~TODO #41~~ | ~~v1.0-gated~~ |
| ~~33~~ | ~~P3~~ done — internal testutil | ~~Move test helpers to `internal/testutil/`~~ | ~~Low~~ | ~~2 hrs~~ | ~~TODO #34~~ | ~~ready~~ |
| ~~34~~ | ~~P3~~ done — go 1.27 | ~~Bump `go.mod` to `go 1.27` when released (removes GOEXPERIMENT)~~ | ~~Med~~ | ~~5 min~~ | ~~blocked on Go 1.27 ship~~ | ~~blocked~~ |
| ~~35~~ | ~~P3~~ done — docs/release-checklist.md | ~~Add a "how to release" runbook to CONTRIBUTING.md~~ | ~~Med~~ | ~~30 min~~ | ~~release tooling~~ | ~~after #1-4~~ |
| ~~36~~ | ~~P3~~ done — changelog lint | ~~Add `CHANGELOG.md` lint (Keep-a-Changelog format validator)~~ | ~~Low~~ | ~~1 hr~~ | ~~docs-health~~ | ~~ready~~ |
| ~~37~~ | ~~P3~~ done (docs-health pass 2026-09-08) | ~~Audit website for broken internal links~~ | ~~Low~~ | ~~30 min~~ | ~~docs-health~~ | ~~ready~~ |
| ~~38~~ | ~~P3~~ done (docs-health pass 2026-09-08) | ~~Annual review of all ADRs (0001–0015) for continued relevance~~ | ~~Low~~ | ~~1 hr~~ | ~~docs-health~~ | ~~ready~~ |
| ~~39~~ | ~~P3~~ done (docs-health pass 2026-09-08) | ~~Audit `docs/feedback/*` for actioned vs open items~~ | ~~Low~~ | ~~30 min~~ | ~~update-old-docs~~ | ~~ready~~ |
| ~~40~~ | ~~P3~~ done — integration/csp nonce test.go | ~~Add CSP test for the new `<search>` landmark wrapper~~ | ~~Low~~ | ~~15 min~~ | ~~full-code-review~~ | ~~ready~~ |
| ~~41~~ | ~~P3~~ done — visualtest | ~~Add golden test for stylable `<select>` across browsers~~ | ~~Low~~ | ~~30 min~~ | ~~full-code-review~~ | ~~ready~~ |
| ~~42~~ | ~~P3~~ done (docs-health pass 2026-09-08) | ~~Verify `field-sizing: content` Baseline-2024 claim in textarea~~ | ~~Low~~ | ~~10 min~~ | ~~full-code-review~~ | ~~ready~~ |
| ~~43~~ | ~~P3~~ done (docs-health pass 2026-09-08) | ~~Sweep for `nolint:` comments that can now be removed~~ | ~~Low~~ | ~~20 min~~ | ~~code-quality-scan~~ | ~~ready~~ |
| ~~44~~ | ~~P3~~ done (docs-health pass 2026-09-08) | ~~Add `examples/demo` `go:generate` directive for CSS rebuild~~ | ~~Low~~ | ~~15 min~~ | ~~demo infra~~ | ~~ready~~ |
| ~~45~~ | ~~P3~~ done — CHANGELOG v1.11.0 | ~~Add `--all` flag to release.sh to run full verify before tagging~~ | ~~Low~~ | ~~20 min~~ | ~~release tooling~~ | ~~after #1-4~~ |
| ~~46~~ | ~~P3~~ done — buildflow repo | ~~Wire `CRUSH_MODEL` env into BuildFlow pre-commit attribution~~ | ~~Low~~ | ~~30 min~~ | ~~release tooling~~ | ~~after #6 (env name)~~ |
| ~~47~~ | ~~P3~~ done — awesome templ pr | ~~awesome-templ PR (updated count)~~ | ~~Low~~ | ~~blocked~~ | ~~TODO #28~~ | ~~external~~ |
| ~~48~~ | ~~P3~~ done — templ guide listing | ~~templ.guide listing~~ | ~~Low~~ | ~~blocked~~ | ~~TODO #29~~ | ~~external~~ |
| ~~49~~ | ~~P3~~ done — ssh tag signing | ~~SSH tag signing config (`gpg.ssh.allowedSignersFile`)~~ | ~~Low~~ | ~~blocked~~ | ~~TODO #30~~ | ~~needs user SSH key~~ |
| ~~50~~ | ~~P3~~ done — docs/adr/0034-targeted-module-split.md | ~~Prototype `display/overlay` + `display/data` package split~~ | ~~Low~~ | ~~4 hrs~~ | ~~go-modularize~~ | ~~v1.0-gated~~ |
...
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------- | ----------------------------------------- |
| ~~1~~    | ~~Read `scripts/release.sh` current state (lines 100-160)~~ done at `f1a2592` | ~~2 min~~ | ~~—~~ | ~~view output matches postmortem~~ |
| ~~2~~    | ~~Edit `release.sh:137` — drop leading `${RELEASE_SUMMARY}\n\n` from `RELEASE_BODY`~~ done at `f1a2592` | ~~2 min~~ | ~~1~~ | ~~`grep RELEASE_BODY` shows no dup~~ |
| ~~3~~    | ~~Edit `release.sh:143` — `Assisted-by: Crush:${CRUSH_MODEL:-unknown}`~~ done at `f1a2592` | ~~3 min~~ | ~~1~~ | ~~`grep Assisted-by` shows `${CRUSH_MODEL}`~~ |
| ~~4~~    | ~~Edit `release.sh:102-109` — add `--notes-file` flag parsing (`case` block)~~ done at `f1a2592` | ~~8 min~~ | ~~1~~ | ~~`bash -n release.sh` syntax OK~~ |
| ~~5~~    | ~~Edit `release.sh:117-125` — when notes empty AND stdin tty, fall back to CHANGELOG `[Unreleased]` body~~ done at `f1a2592` | ~~8 min~~ | ~~4~~ | ~~manual dry-run echoes CHANGELOG body~~ |
| ~~6~~    | ~~`shellcheck scripts/release.sh` (if available)~~ done at `f1a2592` | ~~2 min~~ | ~~5~~ | ~~0 new warnings~~ |
| ~~7~~    | ~~Add `utils/release_commit_test.go` — assert last release commit body has ≥3 paragraphs, `Assisted-by` not `MiniMax-M3` literal~~ done at `f1a2592` | ~~10 min~~ | ~~—~~ | ~~`go test ./utils/...` PASS~~ |
| ~~8~~    | ~~Run full verify: `go build ./... && golangci-lint run … && go test ./...`~~ done at `f1a2592` | ~~2 min~~ | ~~7~~ | ~~green~~ |
| ~~9~~    | ~~`git add scripts/release.sh utils/release_commit_test.go` + commit~~ done at `f1a2592` | ~~5 min~~ | ~~2,3,5,7,8~~ | ~~commit created, BuildFlow passes~~ |
| ~~10~~   | ~~Annotate `docs/status/2026-07-18_09-29_v0.18.0-release-postmortem.md` — append `## Resolution (2026-07-18)` section citing the fix commit~~ done at `f1a2592` | ~~5 min~~ | ~~9~~ | ~~section present~~ |
| ~~11~~   | ~~Regenerate `.art-dupl-baseline.json` (if Q2 = regenerate)~~ done at `f1a2592` | ~~2 min~~ | ~~—~~ | ~~`art-dupl check` → 0 new~~ |
| ~~12~~   | ~~`git push origin master` (2 commits: 24754e5 + the release.sh fix)~~ done — CHANGELOG v0.19.0 | ~~1 min~~ | ~~9, 10~~ | ~~origin matches local~~ |
