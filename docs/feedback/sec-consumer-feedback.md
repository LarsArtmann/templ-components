...
| ------------------------------------------------ | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. Design system mismatch — neon dark aesthetic  | **WON'T FIX**         | Target audience mismatch. The library targets "clean, consistent, professional UI" (SaaS/dashboard/admin panels). SEC's neon-on-black with glassmorphism is a fundamentally different design language. Overriding every component's classes would cost more than maintaining hand-written components. Acknowledged in SEC's own verdict. |
| 2. No game-UI components                         | **WON'T FIX**         | Out of scope. Dice cups, tension bars, streak heatmaps, multiplier displays are domain-specific game UI. No overlap with the library's component catalogue. A separate `templ-components-game` package could be a future project.                                                                                                        |
| 3. CSS-variable model vs SEC's custom variables  | **ALREADY SUPPORTED** | `@theme { --color-blue-600: #...; }` in consumer CSS overrides the library's colors without Go code changes. SEC's neon variables (`--neon-cyan`) can be mapped: `@theme { --color-blue-600: var(--neon-cyan); }`. The mechanism exists; SEC chose not to use it.                                                                        |
| 4. Headless component variants (Radix UI model)  | **DEFERRED to v1.0+** | Major architectural shift — would require a parallel "unstyled" component tree. Not rejected outright, but scoped post-v1.0. The `EnumTable[T]` pattern SEC adopted was the immediate value exchange.                                                                                                                                    |
| 5. Game-UI component pack                        | **WON'T FIX**         | Same as #2 — domain-specific, not generic enough for the library.                                                                                                                                                                                                                                                                        |
| 6. CSS-variable aliasing layer                   | **ALREADY SUPPORTED** | `@theme` overrides in Tailwind v4 CSS-first config are the aliasing layer. See `templ-components-theme.css` for semantic alias examples (`bg-tc-primary`, `text-tc-danger`). No Go mapping layer needed.                                                                                                                                 |
| 7. Slot/render-prop pattern for custom internals | **ALREADY EXISTS**    | `display.Card` has `Footer` and `HeaderAction` `templ.Component` slots + `{ children... }`. `display.PageHeader` has Breadcrumb and Action slots. `navigation.Nav` has `Brand` and `RightItems` slots. The pattern is already the library's composition model.                                                                           |
