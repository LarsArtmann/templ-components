...
| 7    | Full verify: `templ generate` + CSS recompile + build/test/lint, **with local `replace` directives still present** + the govulncheck vulnerability gate (reachable-vuln findings abort the cut; missing binary fails loud) | v1.9.0: stripping replaces before verify made every build fail with `unknown revision <sub>/v<version>` — go1.26 workspace mode still resolves `require` entries at unpushed versions; proven with a 2×2 replaces/GOPRIVATE matrix. Vuln gate: v1.12.0 shipped 4 HIGH npm alerts that no tool ever asked about; the Go 1.26.7 floor exists for the same reason |
...
| 10d  | Run `scripts/check-replace-directives.sh` — it pins the exact replace set per module (also wired into pre-commit + CI, so a miss here means the next push goes red anyway)                                                 | v1.17.0: the step-10 re-add vanished under a daemon race — all 5 dependent sub-modules shipped with NO replace blocks, breaking per-module builds until manually repaired (2026-09-13)                                                                                                                                                                         |
