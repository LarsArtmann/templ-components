...
1. ~~**Commit incrementally** — 36 files uncommitted is the same mistake as the prior session.~~ done (docs-health pass 2026-09-08)
   ~~The fixes are logically independent (forms bugs, display bugs, demo fixes, docs) and~~
   ~~could be 4-5 commits.~~

2. ~~**Audit everything before fixing anything** — Start with a complete sweep, then prioritize.~~ done (docs-health pass 2026-09-08)
   ~~I found bugs in waves because I audited package-by-package instead of all-at-once.~~

3. ~~**Use `nix run .#verify`** — It's the canonical check. The manual equivalent works but~~ done (docs-health pass 2026-09-08)
   ~~skips the Nix-pinned `templ` binary guarantee.~~

4. ~~**Add browser testing** — Several fixes (accordion grid animation, toggle thumb slide,~~ done (docs-health pass 2026-09-08)
...
5. ~~**Add a "dynamic Tailwind class" linter rule** — The Toggle and Pagination bugs were the~~ done — utils/tailwind source test.go
...
6. ~~**The `inert` attribute needs a browser support note** — `inert` is supported in all~~ done (docs-health pass 2026-09-08)
...
7. ~~**Tooltip `aria-describedby` propagation is fragile** — The JS finds the first focusable~~ done (docs-health pass 2026-09-08)
...
8. ~~**Accordion CSS grid needs a `@media (prefers-reduced-transitions)` fallback** — The~~ done — utils/motion compliance test.go
...
9. ~~**The overlay JS generators in `shared.go` are getting complex** — `overlayCloseJS`,~~ done — docs/adr/0014-dialog-migration.md
