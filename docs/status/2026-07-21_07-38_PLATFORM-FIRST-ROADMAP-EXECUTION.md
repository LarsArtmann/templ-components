...
All 5 phases shipped (v0.20.0→v1.1.0). All defects D1–D6 resolved. All tags pushed.

| Item | Claim in report | Resolution |
| ---- | --------------- | ---------- |
| D1 Popover/Dropdown positioning | "probably broken in real browsers" | **CONFIRMED + FIXED in v1.2.0.** `popoverPositionJS` singleton (`display/shared.go`) reads `getBoundingClientRect()` on `toggle` open, sets `style.left/top` with viewport clamping. Used by Popover + Dropdown. ADR-0017 revised. |
| D2 ContextMenu scroll | "probably fine — but didn't verify" | **OK** — `clientX/Y` is correct for top-layer `position: fixed`. No fix needed. |
| D3 Tooltip aria-describedby | "a11y regression" | **FIXED in v1.2.0.** `tooltipAriaJS` singleton re-propagates `aria-describedby` to first focusable child on load + `htmx:afterSettle`. |
| D4 HTMXSrc + response-targets | "defeats half the purpose of self-hosting" | **FIXED in v1.2.0.** Condition now checks `props.HTMXSrc == ""` — self-host suppresses CDN extension. |
| D5 `cmd/tc/_sources/` naming | "non-obvious" | **Documented** in AGENTS.md (leading underscore = `templ generate` skip). |
| D6 `tc add` dependency gap | "copied file won't compile" | **FIXED in v1.2.0.** CLI prints a clear warning after copy pointing to `go get` the full package. |
| G1 Push 5 tags? | "none pushed" | **DONE.** All tags v0.18.1→v1.2.0 pushed to `origin/master`. |
| G2 D1 blocker for tagging? | "options: patch, skip, retag" | **RESOLVED** — shipped fix in v1.2.0 (not a v0.20.1 patch; folded into next minor). |
| G3 Recipes in demo before push? | "invisible without demo routes" | **DONE in v1.2.0.** `/recipes/{dashboard,settings,login}` routes wired into `examples/demo`. |
| C5 DOMAIN_LANGUAGE.md | "not added" | **DONE** — 7 platform terms added (ContainerAware, Recipe, Semantic Token, etc.). |
| C6 TODO_LIST/ROADMAP reconciliation | "never updated to mark closed" | **DONE** — TODO_LIST rebuilt, ROADMAP reconciled (this session). |
