...
1. ~~**Upstream claim verified at the primary source.** User said "v0.3.0 and~~ done at `3c2783d`
...
2. ~~**Baseline established before any change.** `datastar` module tests green~~ done at `3c2783d`
   ~~(GOWORK=off) pre-bump — no pre-existing failures to blame later.~~
3. ~~**Byte-level upstream diff classified breaking-vs-additive BEFORE bumping.**~~ done at `3c2783d`
...
5. ~~**Contract suite green against the new pin (with -race):**~~ done at `3c2783d`
...
6. ~~**Living docs updated.** `docs/datastar-runtime-facts.md` header now records~~ done at `3c2783d`
...
7. ~~**Full local verification matrix green:** 6 sub-modules (GOWORK=off loop) +~~ done at `3c2783d`
...
8. ~~**Upstream-watch quiet state verified post-hoc, not assumed:** `gh run list`~~ done at `3c2783d`
   ...
   ~~see d6).~~
9. ~~**Open question #1 from the 2026-09-02 report is closed** (bump + re-audit~~ done at `3c2783d`
   ~~done same-session-as-asked).~~

## b) PARTIALLY DONE

1. ~~**Canonical done-check skipped:** `nix run .#verify` + `nix flake check` were~~ done (docs-health pass 2026-09-08)
...
2. ~~**`go mod verify` not run** on any module (go-ecosystem-upgrade Phase 4.4).~~ done (docs-health pass 2026-09-08)
   ~~`tidy` ran everywhere; `verify` (go.sum internal consistency) did not.~~
3. ~~**`-race` coverage is partial:** only the 3 datastar contract/drift tests ran~~ done (docs-health pass 2026-09-08)
...
5. ~~**CHANGELOG formatting nit:** my new Changed entry introduced a blank line~~ done (docs-health pass 2026-09-08)
   ~~between entries; the file's convention is consecutive lines. Cosmetic.~~
6. ~~**Bundle-contract hardening identified but not implemented** — the audit~~ done — datastar/bundle guard test.go
...
2. ~~**CI status check for today's three pushed commits** (`bb2d77d`, `04c8adf`,~~ done (CI observed across the release-cut session and after (red jobs repaired 2026-09-03))
   ...
   ~~commit landed.~~
3. ~~**Prior open question #2 is MOOT (record the closure):** "squash the ~21+~~ done (docs-health pass 2026-09-08)
...
4. ~~**Prior open question #3:** changelog-guard policy for test-only PRs —~~ **Won't implement — decided N19.**
   ~~still undecided.~~
...
7. ~~**upstream-watch workflow has never executed** (0 runs since creation) — its~~ done — upstream-watch run 34391305391
...
1. ~~🔴 **Daemon commit `8a9bb87` (today 15:47, 22 files) re-introduced the~~ done (docs-health pass 2026-09-08)
...
3. ~~**False-FAIL root test run:** first verification command ran `GOWORK=off`~~ done (docs-health pass 2026-09-08)
...
4. ~~**multiedit ambiguity failure:** anchored an edit on `"### Changed"` — which~~ done (docs-health pass 2026-09-08)
...
5. ~~**"bumped same day" factual error in CHANGELOG:** discovery was Sep 2, the~~ done (docs-health pass 2026-09-08)
...
6. ~~**Verify-external-claims chat-time gate miss:** I wrote "resolved before its~~ done (docs-health pass 2026-09-08)
...
7. ~~**Carried a wrong premise from the session summary:** "24+ unpushed daemon~~ done (docs-health pass 2026-09-08)
...
3. ~~**Codify the bump protocol** (ls-remote → diff `static/` subtree → sha256~~ done — docs/external-dependency-bumps.md
   ...
   ~~release checklist.~~
4. ~~**Verify the module proxy, not just git tags, when upstream-watch is the~~ done (docs-health pass 2026-09-08)
   ~~consumer.** The workflow compares against proxy `@latest`; a lagging proxy~~
   ~~would false-positive an issue even with tags present.~~
5. ~~**Never encode temporal claims ("same day", "before its first run") without~~ done (docs-health pass 2026-09-08)
   ~~checking the clock / workflow state at write time** (d5, d6).~~
6. ~~**CHANGELOG edits must anchor in unique context** — every release section~~ done (docs-health pass 2026-09-08)
   ~~repeats `### Changed`/`### Added` (d4).~~
7. ~~**Keep a GOWORK=off command cheat sheet** (root-module package list vs~~ done (docs-health pass 2026-09-08)
   ~~sub-module loop) — the false-FAIL (d3) is a recurring command-shape bug.~~
8. ~~**Run the canonical `nix run .#verify` when the module graph moves,** even~~ done (docs-health pass 2026-09-08)
   ~~for "trivial" pin bumps — go.sum resolution is repo-wide.~~
9. ~~**`trash`, not `rm`. Always. Including `/tmp` (d2).~~ done (docs-health pass 2026-09-08)
10. ~~**Re-verify point-in-time premises from session summaries before acting** —~~ done (docs-health pass 2026-09-08)
...
4. ~~`[TODO]` Exercise `upstream-watch.yml` once via `workflow_dispatch` — it has~~ done — upstream-watch run 34391305391
   ~~never run; its jq/proxy parsing is unverified.~~
5. ~~`[TODO]` Document the go-datastar/static bump protocol (e3).~~ done — docs/external-dependency-bumps.md
...
10. ~~`[TODO]` Add provenance block (bundle sha256 + extraction commands) to~~ done — docs/datastar-runtime-facts.md
    ~~`docs/datastar-runtime-facts.md` so re-audits are reproducible.~~
11. ~~`[TODO]` Note the v0.4.0 byte-audit result in the AGENTS.md Datastar blockquote.~~ done — AGENTS.md
12. ~~`[TODO]` Add a version-bump note to `docs/recipes/datastar-integration.md`~~ done — docs/recipes/datastar-integration.md
...
17. ~~`[TODO]` `cmd/tc` scaffolder: embed the bump-protocol checklist in the~~ done — cmd/tc DATASTAR-BUMP-PROTOCOL.md
...
19. ~~`[ROADMAP]` Fuzz `datastarScriptURL`/`resolveDatastarCDN` (URL-shape fuzzing~~ **Won't implement — routed to ROADMAP by the 2026-09-03 harvest.**
    ~~like `FuzzInputType`).~~
20. ~~`[TODO]` Verify SDKScript render is covered by the demo headers-contract test~~ done — examples/demo/sse test.go
...
29. ~~`[TODO]` Changelog-guard policy decision for test-only PRs (c4).~~ **Won't implement — decided N19.**
30. ~~`[TODO]` Record closure of the squash question (daemon pushes; force-push~~ done (docs-health pass 2026-09-08)
...
37. ~~`[TODO]` GOWORK=off cheat sheet → AGENTS.md Build & Test section (e7).~~ done — AGENTS.md
...
40. ~~`[ROADMAP]` Release checklist: add "check daemon didn't regress pins/CSS in~~ done — docs/release-checklist.md
    ~~the window between verify and tag" step.~~
41. ~~`[ROADMAP]` `datastar` package README: state the re-audit contract (facts doc~~ done — datastar/doc.go
    ~~- contract test + sha256 pin) for contributors.~~
42. ~~`[ROADMAP]` Golden sweep: assert `datastarScriptURL` output for CDN + custom~~ done — TestDatastarScriptURL
...
44. ~~`[ROADMAP]` Cross-module DAG test: assert go-datastar/static appears in~~ done — TestGoDatastarStaticPinSurface
...
47. ~~`[ROADMAP]` Docs: single "external dependency bump protocol" page covering~~ done — docs/external-dependency-bumps.md
...
50. ~~`[ROADMAP]` Process: pre-mortem the daemon-push behavior into the release~~ done — docs/release-checklist.md
