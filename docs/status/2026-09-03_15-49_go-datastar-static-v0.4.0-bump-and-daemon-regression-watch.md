...
4. **Post-daemon-commit re-checks (AGENTS.md protocol) partially executed:**
...
1. **Decide the `website/package.json` typescript regression** (daemon commit
...
2. **CI status check for today's three pushed commits** (`bb2d77d`, `04c8adf`,
...
4. **Prior open question #3:** changelog-guard policy for test-only PRs —
   still undecided.
5. **docs-health HARVEST:** the 2026-09-02 report's 50 brainstorm items AND this
   report's section (f) are not yet routed into `TODO_LIST.md` / `ROADMAP.md`.
6. **v1.12.0 release cut** — `[Unreleased]` is warm (~14 entries incl. today's);
...
8. **Push:** one local commit pending (the daemon will push it); house rule
...
1. **Pin the bundle sha256 in `TestPinnedRuntimeBundleContract`.** Today the
...
2. **Add a literal drift guard for the version constant's NAME.**
...
1. `[TODO]` Add bundle sha256 pin to `TestPinnedRuntimeBundleContract` (e1).
2. `[TODO]` Add literal `DatastarVersion1_0_2 == "1.0.2"` drift guard (e2).
3. `[TODO]` Verify proxy `@latest` actually serves `static` v0.4.0 (upstream-watch's
...
6. `[TODO]` Run `go mod verify` on datastar/root/visualtest (b2).
7. `[TODO]` Full `-race` pass over the datastar module (b3).
8. `[TODO]` Run canonical `nix run .#verify` + `nix flake check` (b1).
...
13. `[ROADMAP]` Bump the actual Datastar runtime when starfederation/datastar
    ships >1.0.2 — bundle bytes WILL change; full facts-doc re-audit required.
14. `[ROADMAP]` Upgrade `datastar-sse-error` handling if a future bundle ever
    dispatches it (contract test currently only logs; decide a policy).
15. `[ROADMAP]` Upstream watch: also track starfederation/datastar releases
    directly, not just our wrapper module.
16. `[ROADMAP]` upstream-watch: proxy-lag grace before opening an issue (f3).
17. `[TODO]` `cmd/tc` scaffolder: embed the bump-protocol checklist in the
    generated datastar package docs.
18. `[ROADMAP]` Demo: surface the pinned runtime version in the datastar demo UI.
19. `[ROADMAP]` Fuzz `datastarScriptURL`/`resolveDatastarCDN` (URL-shape fuzzing
    like `FuzzInputType`).
...
21. `[ROADMAP]` visualtest: LiveRegion busy-state golden with a real SSE stream
    (pixel coverage of `aria-busy`).
22. `[ROADMAP]` Recipe: computing SRI hashes for self-hosted bundles (doc.go says
    "compute your own" — show how).

**From the daemon regression discovery (urgent cluster):**

23. `[TODO]` **Decide typescript `^7.0.2` flip: verify astro `^7.3.1` + TS 7
    compatibility, then revert to `^6.0.3` or keep** (c1/d1 — top priority).
24. `[TODO]` Re-run `nix run .#css` and restore byte-stable `app.css` if the
    +4951-line rebuild fails the CSS Freshness check.
25. `[TODO]` Check CI status for today's pushed commits (c2).
26. `[ROADMAP]` Fix BuildFlow upstream (`larsartmann/buildflow`): stop the
...
27. `[TODO]` Confirm `.golangci.yml` `run.go: 1.26.7` change passes
...
28. `[TODO]` docs-health HARVEST: 2026-09-02 report's 50 items + this report's
...
31. `[TODO]` v1.12.0 release cut when `[Unreleased]` is finalized (c6).
32. `[TODO]` Post-release: re-add replaces + GOWORK=off tidy sweep in all 8
    modules (release-checklist step).
33. `[ROADMAP]` GitHub Actions badge/status monitoring for master after daemon
    push storms.
34. `[ROADMAP]` Daemon commit-message quality (it template-generates; no
    diff-stat) — upstream BuildFlow fix, sibling of #26.
35. `[ROADMAP]` Daemon commit classifier: never auto-commit `website/package.json`
    or `examples/demo/static/app.css` without the byte-stability gate.

**Smaller hygiene observed this session:**

36. `[TODO]` `trash` the leftover `/tmp/go-datastar-audit` clone (d2 cleanup).
37. `[TODO]` GOWORK=off cheat sheet → AGENTS.md Build & Test section (e7).
38. `[ROADMAP]` Consider `workflow_dispatch` triggers on all cron workflows for
    manual exercise (upstream-watch, master-red-alert).
39. `[ROADMAP]` Proxy-lag monitoring: weekly job asserting proxy `@latest` ==
...
43. `[ROADMAP]` Consider surfacing `static.Version` in SDKScript as a data
...
44. `[ROADMAP]` Cross-module DAG test: assert go-datastar/static appears in
    ...
    drift guard.
45. `[ROADMAP]` Upstream (verify-before-filing first): propose `static` module
    release automation so bundle refreshes ride datastar releases automatically.
46. `[ROADMAP]` Consider depending on `go-datastar/static` via `@latest`-pin
...
48. `[ROADMAP]` visualtest: `-run` filter documentation for the datastar goldens.
49. `[ROADMAP]` Demo: reconnection UX showcase for `LiveRegionProps.Retry`
...
1. **`website/package.json`: the daemon flipped `typescript` to `^7.0.2` (the
   ...
   your calls, not mine.
2. **The daemon's un-minified `app.css` rebuild (+4951 lines) is the third
