...
- A visual golden for the dashboard *recipe page* (not just components) would have caught the original collapse years earlier; "page-level goldens" are not a thing yet in this repo.
...
*Report per the status-report skill. Format override (`.md` over the canonical HTML dashboard) honored per user instruction. Section (f) remains the primary input for a future docs-health HARVEST.*
