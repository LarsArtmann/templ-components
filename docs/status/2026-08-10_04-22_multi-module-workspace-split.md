...
| #      | Item                                                                                                                          | Evidence                                                                 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| ~~1~~  | ~~**4 new `go.mod` files created** (utils, icons, errorpage, charts/echarts)~~ done at `d7ac9f3d`                             | ~~Each has correct deps + replace directives; `go.sum` files generated~~ |
| ~~2~~  | ~~**Root `go.mod` updated** with require + replace for all 4 sub-modules~~ done at `d7ac9f3d`                                 | ~~`go build ./...` passes~~                                              |
| ~~3~~  | ~~**`internal/svg` → `utils/svg`** (git mv + all imports updated)~~ done at `d7ac9f3d`                                        | ~~10 source files + 7 CLI source copies updated~~                        |
| ~~4~~  | ~~**`internal/cdn` → `utils/cdn`** (git mv + imports updated)~~ done at `d7ac9f3d`                                            | ~~2 source files updated~~                                               |
| ~~5~~  | ~~**`internal/golden` → `utils/golden`** (git mv + imports updated)~~ done at `d7ac9f3d`                                      | ~~~25 test files across 8 packages updated~~                             |
| ~~6~~  | ~~**`go.work` fixed** — removed absolute path `/home/lars/projects/go-error-family`, added all 5 modules~~ done at `d7ac9f3d` | ~~Portable, machine-independent~~                                        |
| ~~7~~  | ~~**`.envrc` updated** — removed `GOWORK=off` (workspace now active by default)~~ done at `d7ac9f3d`                          | ~~direnv loads workspace for all tools~~                                 |
| ~~8~~  | ~~**`flake.nix` shellHook updated** — removed `GOWORK=off`~~ done at `d7ac9f3d`                                               | ~~devShell now uses workspace mode~~                                     |
| ~~9~~  | ~~**`flake.nix` apps updated** — test, lint, verify now handle multi-module~~ done at `d7ac9f3d`                              | ~~Per-module lint with GOWORK=off; workspace test~~                      |
| ~~10~~ | ~~**CI workflow updated** — multi-module lint, per-module isolation tests~~ done at `d7ac9f3d`                                | ~~`.github/workflows/ci.yaml`~~                                          |
| ~~11~~ | ~~**`TestEnvrcConsistency` updated** — removed `GOWORK=off` assertion~~ done at `d7ac9f3d`                                    | ~~Passes~~                                                               |
| ~~12~~ | ~~**gci import ordering fixed** — 13 test files re-formatted~~ done at `d7ac9f3d`                                             | ~~`goimports -w` applied~~                                               |
| ~~13~~ | ~~**All 5 modules build standalone** (`GOWORK=off go build ./...`)~~ done at `d7ac9f3d`                                       | ~~Verified per-module~~                                                  |
| ~~14~~ | ~~**All 5 modules test standalone** (`GOWORK=off go test ./...`)~~ done at `d7ac9f3d`                                         | ~~All pass~~                                                             |
| ~~15~~ | ~~**All 5 modules lint clean** (`golangci-lint run`)~~ done at `d7ac9f3d`                                                     | ~~0 issues across all modules~~                                          |
| ~~16~~ | ~~**templ regenerated** (107 files)~~ done at `d7ac9f3d`                                                                      | ~~`templ generate ./...` clean~~                                         |
| ~~17~~ | ~~**AGENTS.md updated** — module structure table, import graph, lint command, build commands~~ done at `d7ac9f3d`             | ~~Reflects new 5-module workspace~~                                      |
| ~~18~~ | ~~**DAG verified** — strict acyclic dependency graph~~ done at `d7ac9f3d`                                                     | ~~utils(leaf) ← icons ← errorpage; utils ← charts/echarts; all ← root~~  |
...
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~1~~ | ~~**Compatibility shim**~~ done — sub-module tags published at releases (check-release-tags.sh); go.mod requires real versions, no placeholders | ~~Root module `require`s all sub-modules; local dev + repo clone consumers work~~ | ~~**Proxy consumers will fail** — `v0.0.0` require with `replace` is ignored by `go get` from proxy.golang.org. Sub-module packages need published version tags OR the root module needs re-export alias packages. This is the biggest gap.~~ |
...
| ~~3~~ | ~~**CI workflow**~~ done — ci.yaml runs scripts/check-module-sync.sh; utils TestGoWorkDirectiveMatchesRootGoMod                                 | ~~Lint + test updated for multi-module~~                                          | ~~No go.work/replace sync check; no version-drift detection script~~                                                                                                                                                                          |
| ~~4~~ | ~~**flake.nix**~~ done — flake.nix coverage app runs per-module (all modules)                                                                   | ~~shellHook, test, lint, verify apps updated~~                                    | ~~coverage app not updated (still single-module `go test ./...`)~~                                                                                                                                                                            |
...
| #      | Item                                                                                                                                  | Impact                                                                                                                             |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| ~~1~~  | ~~**ADR-0020 update**~~ done — docs/adr/0020 marked Superseded by ADR-0034                                                            | ~~Still says "Proposed — deferred until consumer demand." Must be marked "Accepted — executed" or superseded.~~                    |
...
| ~~9~~  | ~~**`scripts/release.sh` multi-module support**~~ done — release.sh tags all 7 modules (docs/modularization/README.md)                | ~~Still single-module. Needs multi-module tagging (e.g., `utils/v2.0.0`, `icons/v2.0.0` etc.) or documented shared-tag approach.~~ |
| 10     | **`scripts/pre-commit.sh`**                                                                                                           | Still exports `GOWORK=off` (line 7). Needs update for workspace mode.                                                              |
| ~~11~~ | ~~**Versioning strategy implementation**~~ done — shared versioning scripted: release.sh multi-module tagging + check-release-tags.sh | ~~Shared versioning chosen but no tagging strategy documented or scripted.~~                                                       |
| ~~12~~ | ~~**flake.nix coverage app**~~ done — flake.nix coverage runs per-module                                                              | ~~Still `go test ./... -coverprofile` (root module only). Misses sub-module coverage.~~                                            |
| ~~13~~ | ~~**go.work / replace sync CI check**~~ done — ci.yaml Module-sync guard step present                                                 | ~~Skill recommends a CI check verifying go.work `use` directives match actual `go.mod` files. Not added.~~                         |
...
3. ~~**The `v0.0.0` placeholder in require blocks is a time bomb.** It works now (replace~~ done (go.mod files use real tagged versions; no v0.0.0 placeholders remain)
...
4. ~~**DAG layer assignment should be documented as a CI-enforced script.** The skill recommends~~ done (scripts/check-module-layers.sh exists (pre-commit Guard 5 + ci.yaml))
...
1. ~~Fix the compatibility shim — decide approach A/B/C and implement~~ done (compat via multi-tag releases: real versions + sub-module tags in go.mod)
2. ~~Update `scripts/release.sh` for multi-module tagging~~ done (scripts/release.sh does multi-module tagging (all 7 modules))
3. ~~Design and document the versioning workflow (shared tag format)~~ done (docs/modularization/README.md documents shared versioning + release process)
4. ~~Update root `go.mod` require entries from `v0.0.0` to real version at release time~~ done (root go.mod requires real versions (no v0.0.0 found))
5. ~~Add CI check: go.work sync idempotency (`go work sync && git diff --exit-code`)~~ done (ci.yaml module-sync guard step)
6. ~~Add CI check: replace directive portability (no absolute paths)~~ done (scripts/check-module-sync.sh verifies relative replace paths (CI + pre-commit Guard 4))
7. ~~Add CI check: version drift detection across modules~~ done (check-module-sync.sh + check-version-sync.sh verify version consistency)
...
9. ~~Update ADR-0020 status from "Proposed — deferred" to "Accepted — superseded by execution"~~ done (ADR-0020 marked Superseded by ADR-0034)
10. ~~Write new ADR for the actual executed split (module boundaries, versioning, compat strategy)~~ done (docs/adr/0034-targeted-module-split.md exists)
11. ~~Update `scripts/pre-commit.sh` — remove `GOWORK=off` or document why it stays~~ done (scripts/pre-commit.sh rewritten per-module (7-module loop))
...
15. ~~Update skill/SKILL.md — `internal/svg` → `utils/svg` references~~ done (skill/SKILL.md uses utils/svg, no internal/svg)
16. Refresh stale `docs/modularization/PROPOSAL.md` (2026-05-14 version)
17. ~~Update `docs/modularization/DEPENDENCY_GRAPH.md` with current DAG~~ done (DEPENDENCY_GRAPH.md deleted; DAG documented in ADR-0034/AGENTS.md + docs/diagrams)

### Medium priority (polish + correctness)

18. ~~Update `flake.nix` coverage app for multi-module coverage aggregation~~ done (flake.nix coverage app per-module (all modules))
19. Add per-module `.golangci.yml` or verify inheritance works
20. ~~Verify Dockerfile pipeline works with multi-module (no Dockerfile exists currently, but~~ done (examples/demo/Dockerfile exists (AGENTS.md section accurate))
...
22. ~~Update `docs/icons-only-adoption.md` — now `go get .../icons` is possible!~~ done (docs/icons-only-adoption.md Standalone module section with go get icons@latest)
23. ~~Update `docs/migration/` guides for the v2.0 import path changes~~ done (docs/migration/v1-to-v2.md exists)
24. ~~Add a DAG layer enforcement script (`scripts/check-module-layers.sh`)~~ done (scripts/check-module-layers.sh exists)
25. ~~Add a module sync verification script~~ done (scripts/check-module-sync.sh exists)
26. ~~Run full race-enabled test suite (`go test -race ./...`) per module~~ done (ci.yaml per-module GOWORK=off go test -race)
27. Run visual regression tests to verify nothing visually broke
28. ~~Update CHANGELOG `[Unreleased]` section with the modularization changes~~ done (CHANGELOG.md ADR-0034 modularization entry)
29. ~~Update ROADMAP.md — mark modularization as done, update next steps~~ done (ROADMAP.md Module structure row (ADR-0034))
30. ~~Update TODO_LIST.md — add multi-module-specific tasks~~ done (TODO_LIST.md contains multi-module items)

### Lower priority (nice to have)

31. Consider extracting `utils/svg` as its own module (zero deps, pure SVG data)
32. ~~Consider extracting `htmx` as its own module (thin, depends only on utils)~~ done (htmx/go.mod exists (extracted as module))
33. ~~Consider extracting `datastar` as its own module (depends on utils/cdn + utils)~~ done (datastar/go.mod exists (extracted as module))
34. ~~Document the module dependency DAG as a D2 diagram in docs/~~ done (docs/diagrams/*.d2 diagrams exist)
35. Add a `Module Structure` section to README with a visual diagram
36. ~~Consider a `go.work` template for contributors (since it's gitignored)~~ done (docs/modularization/README.md documents go.work creation for contributors)
37. ~~Add contributor docs for "how to add a new sub-module"~~ done (docs/modularization/README.md contributor setup)
38. Verify `cmd/tc` CLI scaffolding tool works with multi-module structure
39. ~~Update `cmd/tc/_sources/` template files to reference new import paths (done via sed, verify)~~ done (cmd/tc/_sources files import utils/svg (updated paths))
...
44. ~~Update `docs/testing-guide.md` — golden test paths changed~~ done (docs/testing-guide.md clean of internal/ refs)
45. ~~Update `docs/visual-testing.md` — visualtest module setup unchanged but verify~~ done (docs/visual-testing.md clean of internal/ refs)
