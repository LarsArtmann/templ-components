...
4. **CSS freshness assurance.** Works: all 13 new static class tokens + dynamic Grid/Button/InlineSuccess classes verified present in the _committed_ CSS. Remains: did not run `nix run .#css` byte-stability recompile, so CI's CSS Freshness content-diff is unconfirmed locally. Effort: S.
