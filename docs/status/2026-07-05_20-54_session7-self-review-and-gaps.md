...
| ----- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| ~~1~~ | ~~Replace `containsChar` with `strings.Contains` in LoadMore~~ done — navigation/loadmore.templ                | ~~✅ Done — `containsChar` deleted, `net/url` used~~ |
| ~~2~~ | ~~Fix contract test comment counts (display 18→23, nav 6→7)~~ done — internal/contract/component props test.go | ~~⬠ Stale comments remain (cosmetic only)~~          |
| ~~3~~ | ~~CopyButton `aria-live` for "Copied!" screen reader feedback~~ done — display/copy button.templ               | ~~✅ Done — `role="status"` + `aria-live="polite"`~~ |
| ~~4~~ | ~~Golden test for `RelativeTime` with `AutoRefresh: true` path~~ done — display/testdata                       | ~~✅ Done~~                                          |
| ~~5~~ | ~~Integration tests: CopyButton+Card, CountBadge+Button compositions~~ done — integration/composition test.go  | ~~✅ Done — 7 composition tests~~                    |
| ~~6~~ | ~~SKILL.md Part 2: document CopyButton/Image/CountBadge JS patterns~~ done — skill/SKILL.md                    | ~~✅ Done — full SKILL.md rewrite~~                  |
...
| #     | Task                                                                                                                               | Impact                         | Effort    |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | --------- |
| ~~1~~ | ~~Fix contract test comment counts (display 18→23, nav 6→7)~~ done — internal/contract/component props test.go                     | ~~Accuracy~~                   | ~~2 min~~ |
| ~~2~~ | ~~CopyButton: add `aria-live="polite"` to label span~~ done — display/copy button.templ                                            | ~~A11y~~                       | ~~3 min~~ |
| ~~3~~ | ~~Delete `containsChar`, replace with `strings.Contains`~~ done — navigation/loadmore.templ                                        | ~~Code quality~~               | ~~3 min~~ |
| ~~4~~ | ~~Fix `feedback/loading.templ:119` templ minmax hint~~ done — feedback/loading.templ                                               | ~~Lint cleanliness~~           | ~~3 min~~ |
| ~~5~~ | ~~Add `CursorParam` field to LoadMoreProps (default "cursor")~~ **Won't implement — cursor param removed by later wire redesign.** | ~~API parity with Pagination~~ | ~~5 min~~ |
| ~~6~~ | ~~RelativeTime: add golden test for `AutoRefresh: true` path~~ done — display/testdata                                             | ~~Test coverage~~              | ~~5 min~~ |
| 7     | CountBadge: add `Max=0` edge case test (verify default 99)                                                                         | Edge case coverage             | 5 min     |
| ~~8~~ | ~~Delete stale LSP error on `golden_new_test.go` (restart LSP)~~ **Won't implement — transient lsp issue moot.**                   | ~~Dev experience~~             | ~~1 min~~ |

### Medium Effort (10-30 min)

| #      | Task                                                                                                                | Impact                   | Effort     |
| ------ | ------------------------------------------------------------------------------------------------------------------- | ------------------------ | ---------- |
| ~~9~~  | ~~LoadMore: use `net/url` pattern from Pagination (delete string concat)~~ done — navigation/loadmore.templ         | ~~Correctness + parity~~ | ~~15 min~~ |
| ~~10~~ | ~~Integration tests: CopyButton inside Card, CountBadge wrapping Button~~ done — integration/composition test.go    | ~~Composition coverage~~ | ~~15 min~~ |
| ~~11~~ | ~~SKILL.md Part 2: document CopyButton/Image/RelativeTime JS patterns~~ done — skill/SKILL.md                       | ~~Maintainer guidance~~  | ~~20 min~~ |
| ~~12~~ | ~~CopyButton: add `document.execCommand('copy')` fallback for old browsers~~ done — display/copy button.templ       | ~~Compatibility~~        | ~~15 min~~ |
| ~~13~~ | ~~StatCard HTMX: golden test for `hx-get` variant~~ done — display/testdata                                         | ~~Snapshot coverage~~    | ~~10 min~~ |
| ~~14~~ | ~~Card.Body: golden test for Body slot variant~~ done — display/testdata                                            | ~~Snapshot coverage~~    | ~~10 min~~ |
| ~~15~~ | ~~Demo: anchor-linked table of contents at top~~ done — examples/demo/demo.templ                                    | ~~Demo navigability~~    | ~~15 min~~ |
| ~~16~~ | ~~Add `formatRelativeTime` boundary table test (minute/hour/day/week edges)~~ done — display/coverage extra test.go | ~~Logic coverage~~       | ~~20 min~~ |

### Larger Effort (30 min+)

| #      | Task                                                                                                                  | Impact                   | Effort     |
| ------ | --------------------------------------------------------------------------------------------------------------------- | ------------------------ | ---------- |
| ~~17~~ | ~~Image: handle `srcset` in fallback swap (not just `src`)~~ done — display/image.templ                               | ~~Correctness~~          | ~~30 min~~ |
...
| ~~20~~ | ~~CopyButton: add `Href` variant (link button that also copies)~~ done — display/copy button.templ                    | ~~Use case~~             | ~~20 min~~ |
| ~~21~~ | ~~LoadMore: add infinite-scroll variant (`hx-trigger="revealed"`)~~ done — navigation/loadmore.templ                  | ~~UX feature~~           | ~~25 min~~ |
| ~~22~~ | ~~Benchmark tests for all 6 new components~~ done — display/benchmark test.go                                         | ~~Performance baseline~~ | ~~20 min~~ |
| ~~23~~ | ~~Dedicated forms quickstart demo route (`/forms`)~~ done — examples/demo/forms demo.templ                            | ~~Discoverability~~      | ~~30 min~~ |
| ~~24~~ | ~~ADR: HATEOAS-first philosophy (formalize the shift from "zero JS")~~ done — docs/adr/0007-self-host-htmx-default.md | ~~Architecture record~~  | ~~20 min~~ |
| ~~25~~ | ~~Audit all components for `aria-live` opportunities beyond CopyButton~~ done — feedback/toast.templ                  | ~~A11y audit~~           | ~~30 min~~ |
