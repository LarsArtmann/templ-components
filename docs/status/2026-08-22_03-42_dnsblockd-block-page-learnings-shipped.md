...
| #      | Item                                                                                                                                                                                                                                                                                                                                                                             | Evidence                                                                                                                           |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| ~~1~~  | ~~`display.Eyebrow` component (props, defaults, empty-text guard, `Class` accent override)~~ done at `9266dcb`                                                                                                                                                                                                                                                                   | ~~`display/eyebrow.templ` + `eyebrow_templ.go` (committed, tracked)~~                                                              |
| ~~2~~  | ~~`display.Scrollback` + `ScrollbackTone` typed enum (5 tones, graceful unknown-value fallback, `Stagger` opt-in via `DefaultScrollbackProps`, decorative `aria-hidden` default / `AriaLabel` opt-in)~~ done at `9266dcb`                                                                                                                                                        | ~~`display/scrollback.templ`~~                                                                                                     |
| ~~3~~  | ~~Stagger CSS: `.tc-log`, `.tc-log-line` nth-child delays (8-entry cap + `n+9` catch-all), `.tc-log-line-still` off-switch, `prefers-reduced-motion` neutralization, `overflow-wrap: anywhere`~~ done at `9266dcb`                                                                                                                                                               | ~~`templates/custom.css` (~68 lines added)~~                                                                                       |
| ~~4~~  | ~~Full test lenses for both components: golden sweep (7 goldens), behavior/BDD-style, a11y (aria-hidden vs aria-label, dark variants, reduced-motion CSS guard), edge cases (empty, unknown tone, no-tag lines, 12-line render, no-physical-RTL check), godoc examples, `ScrollbackToneIsValid` + enum table entry, tone-map dark: coverage test~~ done at `9266dcb`             | ~~`display/eyebrow_test.go`, `display/scrollback_test.go`, `display/example_eyebrow_scrollback_test.go`, `display/enums_test.go`~~ |
| ~~5~~  | ~~Contract registration (`EyebrowProps{}`, `ScrollbackProps{}` in `componentTypes()`)~~ done at `9266dcb`                                                                                                                                                                                                                                                                        | ~~`internal/contract/component_props_test.go`~~                                                                                    |
| ~~6~~  | ~~Demo showcase section ("Eyebrow & Scrollback", 5-line DNS trace) + hero count 116→118 + demo CSS recompiled and verified (all new classes present in minified output)~~ done at `9266dcb`                                                                                                                                                                                      | ~~`examples/demo/display_demo.templ`, `demo.templ`, `static/app.css`~~                                                             |
| ~~7~~  | ~~All count-guarded docs updated in lockstep: FEATURES.md (118 components, 54 enums/51 IsValid, 116 generated files, display 40→42 + 2 component rows), README (3 count sites + catalogue entries + code sample), ROADMAP, AGENTS.md (package table + generated-files count), skill/SKILL.md (count, display 38→40, 2 catalogue rows), website `sections.ts`~~ done at `9266dcb` | ~~`TestDocsCountDrift`, `TestHeroCountsMatchFeatures` pass~~                                                                       |
| ~~8~~  | ~~CHANGELOG `[Unreleased]` warmed (3 entries: Eyebrow, Scrollback, docs)~~ done at `9266dcb`                                                                                                                                                                                                                                                                                     | ~~`CHANGELOG.md`~~                                                                                                                 |
| ~~9~~  | ~~Adoption guide: new "Deterministic scanning" section (`source(none)`, `@source not *_templ.go`, exclude CSS outputs) + vendored-path gotcha callout linking the recipe~~ done at `9266dcb`                                                                                                                                                                                     | ~~`docs/tailwind-v4-adoption-guide.md`~~                                                                                           |
| ~~10~~ | ~~New recipe: `docs/recipes/vendored-tailwind-scanning.md` (class-inventory pattern, env comparison table, drift-guard guidance)~~ done at `9266dcb`                                                                                                                                                                                                                             | ~~committed~~                                                                                                                      |
| ~~11~~ | ~~New recipe: `docs/recipes/split-identity-page.md` (two-zone composition, ingredient table, 6 design rules incl. one-motion-moment and data-tone switching) + recipe-index entries~~ done at `9266dcb`                                                                                                                                                                          | ~~committed~~                                                                                                                      |
| ~~12~~ | ~~theme-bridge.md: warm-paper light-mode trick + single-signal-accent pattern (both proven in dnsblockd)~~ done at `9266dcb`                                                                                                                                                                                                                                                     | ~~`docs/recipes/theme-bridge.md`~~                                                                                                 |
| ~~13~~ | ~~DOMAIN_LANGUAGE.md: Eyebrow, Scrollback, ScrollbackTone glossary rows~~ done at `9266dcb`                                                                                                                                                                                                                                                                                      | ~~committed~~                                                                                                                      |
| ~~14~~ | ~~**Pre-existing failure fixed:** `TestReleaseScriptInvariants` expected the pre-v1.9.0 `release_rollback` hook name; script had correctly renamed it `release_cleanup` in the v1.9.0 single-EXIT-trap fix. Test updated to pin the real invariant.~~ done at `9266dcb`                                                                                                          | ~~`utils/release_script_test.go`~~                                                                                                 |
| ~~15~~ | ~~**Pre-existing failure fixed:** ROADMAP.md component count drifted (116 vs actual) — updated to 118~~ done at `9266dcb`                                                                                                                                                                                                                                                        | ~~`ROADMAP.md`~~                                                                                                                   |
| ~~16~~ | ~~Full verification: `nix run .#verify` → "All checks passed" (generate + build + test + lint, root + all sub-modules, 0 lint issues); `nix flake check` → all checks passed; per-module isolation (GOWORK=off) for icons/errorpage/charts/echarts/htmx/datastar → green; `nix fmt` applied~~ done at `9266dcb`                                                                  | ~~session log~~                                                                                                                    |
| ~~17~~ | ~~BuildFlow gotcha handled: new `*_templ.go` force-staged (`git add -f`), final `.gitignore` check shows only the `!*_templ.go` unignore (no re-added blocker); tree clean, all work committed~~ done at `9266dcb`                                                                                                                                                               | ~~`git status` clean~~                                                                                                             |
...
3. ~~**Split-identity-page recipe precision** — the ingredient table suggests~~ done — docs/recipes/split-identity-page.md
...
4. ~~**utils per-module isolation test** — ran utils tests in workspace mode only; the~~ done (docs-health pass 2026-09-08)
...
5. ~~**Stale-count sweep** — I updated every doc the drift test guards, but did not grep~~ done — utils/docs count test.go
...
6. ~~**Website verification** — edited `website/src/data/sections.ts` (two count strings)~~ done — .github/workflows/website.yml
...
1. ~~**Visual regression goldens** for Eyebrow + Scrollback (`nix run .#visual`,~~ done — visualtest/testdata/eyebrow
...
3. ~~**Release cut** — CHANGELOG is warm; no v1.10.0 tagged (correctly — not asked).~~ done — CHANGELOG v1.10.0
...
6. ~~**Dark-mode golden variants** — `display/dark_golden_test.go` covers other~~ done — visualtest/testdata/scrollback
...
1. ~~**Wrote tests asserting ordered class substrings** (`"text-gray-500 dark:text-gray-400"`)~~ done (docs-health pass 2026-09-08)
...
2. ~~**Malformed doc-comment example** in `scrollback.templ` v1 (`DefaultScrollbackProps()`~~ done (docs-health pass 2026-09-08)
   ~~followed by stray `Lines:` lines). Caught on re-read before generation; fixed.~~
3. ~~**Two edit-tool refusals** for editing files without reading them first~~ done (docs-health pass 2026-09-08)
...
4. ~~**9 lint findings** (8× wsl_v5 whitespace, 1× golines) in my new test files — I~~ done (docs-health pass 2026-09-08)
...
5. ~~**GOCACHE detour** — first tried `~/.cache/go-build`, which is a stale home-manager~~ done (docs-health pass 2026-09-08)
...
6. ~~**One malformed whitespace line** in scrollback_test.go (`styles := ...`) shipped in~~ done (docs-health pass 2026-09-08)
...
1. ~~**Per-package lint immediately after writing tests** — all 9 findings would have~~ done (docs-health pass 2026-09-08)
   ~~been caught in seconds at authoring time instead of at verify time.~~
2. ~~**A per-package count drift guard** — component-count updates required touching~~ done — utils/docs count test.go
...
4. ~~**The environment is half-broken** — `/mnt/buildcache` autofs dead, LSP polluted by~~ done (docs-health pass 2026-09-08)
...
1. ~~Cut library release v1.10.0 via `scripts/release.sh` (CHANGELOG is warm; verify runs clean)~~ done — CHANGELOG v1.10.0
...
7. ~~Add visual goldens for Eyebrow + Scrollback (final state, light+dark) via `nix run .#visual`~~ done — visualtest/testdata/eyebrow
...
12. ~~Add dark-golden variants for both components~~ done — visualtest/testdata/scrollback
...
17. ~~Fix the Split-ratio imprecision in split-identity-page.md ("closest", plus when to use the arbitrary grid)~~ done — docs/recipes/split-identity-page.md
...
22. ~~Extend `TestDocsCountDrift` to per-package component counts (display/forms/etc. rows)~~ done — utils/docs count test.go
23. ~~Run utils module isolation test (`GOWORK=off`) — close the loop from (b)~~ done — utils/docs count test.go
24. ~~Sweep long-tail docs (CONTRIBUTING.md, docs/*.md) for stale "116"/"40" counts~~ done — utils/docs count test.go
25. ~~Run website `astro check` to validate the sections.ts edit~~ done — .github/workflows/website.yml
...
32. ~~Document the "one motion moment per page" rule as an ADR or design-principle note~~ done — docs/recipes/split-identity-page.md
...
1. ~~**Release timing:** cut **v1.10.0** now (2 components + docs), or hold and bundle~~ **Won't implement — cut v1.10.0.**
...
3. ~~**Visual goldens priority:** add them for the two new components now (requires a~~ **Won't implement — added 09-02.**
