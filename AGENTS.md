...
> **Datastar runtime facts (2026-08-21 audit, re-audited for v1.0.3 on 2026-09-05):** see `docs/datastar-runtime-facts.md` — only `datastar-patch-*` SSE events (keyed datalines, blank line terminates), lifecycle errors only on `datastar-fetch` (no `datastar-sse-error`), CSP needs `'unsafe-eval'`, clean stream EOF only reconnects under `retry: 'always'` (`LiveRegionProps.Retry`). v1.0.3 added a client-side `selector` fetch option; `wire.Action` still renders Target htmx-only (ADR-0036 stands). Wire format pinned by `examples/demo/sse_test.go`; bundle contract pinned by `datastar.TestPinnedRuntimeBundleContract`. e2e-discovered runtime fact (2026-09-07): Datastar inner-mode patches MORPH swapped subtrees and preserve form input values across DIFFERENT positionally-corresponding fields — server step machines must accept unexpectedly non-empty fields, and e2e flows must clear fields explicitly before submitting.
