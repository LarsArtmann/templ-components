...
| ------------------- | ------------------------------------------ | ------------------------------------------------------------------------- |
| `display`           | 27 UI components                           | Cards, tables (Table + DataTable), modals, badges, buttons, avatars, etc. |
| `feedback`          | 13 components                              | Alerts, toasts, spinners, skeletons, progress bars                        |
| `forms`             | 19 components                              | Inputs, selects, toggles, combobox, slider, rating, validation            |
...
- 69 generated files across all packages (display, errorpage, feedback, forms, htmx, icons, layout, navigation, examples/demo, internal/golden)
...
- **Overlay JS must sync `aria-hidden` and `inert`.** The server render sets `aria-hidden`/`inert` based on `Open`, but the JS open/close functions (`overlayOpenJS`/`overlayCloseJS` in `display/shared.go`) must also toggle these attributes so dynamically opened/closed overlays stay accessible. A JS-opened modal that's still `inert` is keyboard-invisible.
...
- **Accordion CSS grid technique**: uses `grid-rows-[1fr]`/`grid-rows-[0fr]` (CSS grid auto-height animation) instead of `max-h-96` (384px cap). The old approach clipped content taller than 384px with no scroll fallback. The JS toggles these grid-rows classes.
