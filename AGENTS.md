...
- **Dropdown/Popover/ContextMenu use native Popover API (ADR-0017)**: `popover="auto"` + declarative `popovertarget` on the trigger button replaces ~70 lines of singleton JS. Native light-dismiss (click-outside + Escape), top-layer rendering, focus restore. `Popover` is now zero-JS. `Dropdown` keeps ~25 lines of singleton JS for WAI-ARIA menu keyboard nav (ArrowUp/Down/Home/End + RTL mapping) and focuses first menuitem on `toggle` event. `ContextMenu` keeps ~6 lines of singleton JS for the `contextmenu` event → `showPopover()` + cursor-position `inset`. CSS positioning (`bottom-full` / `top-full` / etc.) is retained — Anchor Positioning is Baseline 2026 but offers no win over the existing class-based positioning for these components. `[popover]::backdrop { background-color: transparent; }` in `templates/custom.css` ensures popovers don't dim the page (menus aren't modals).
