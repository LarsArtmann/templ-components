...
- 75 generated files across all packages
...
- Modal/Drawer: native `<dialog>` element provides focus trapping, Escape-to-close, focus restore, top-layer rendering, and `::backdrop` — zero JS for those behaviors. CSS `@starting-style` + `allow-discrete` handle open/close animations (defined in `templates/app.css` under `.tc-modal` / `.tc-drawer`). `tcOpenModal(id)` / `tcCloseModal(id)` are thin wrappers around `dialog.showModal()` / `dialog.close()` for backward compat. Backdrop click detection: `e.target === dialog` (the backdrop is a pseudo-element of dialog, so clicks register on the dialog itself).
...
- **Drawer:** `display.Drawer` — accessible side panel rendered as a native `<dialog>` with `data-side="left"`/`"right"`. CSS positions the dialog via `margin-inline-*` (auto-mirrors in RTL) and animates via `translateX`. Side positioning is in `templates/app.css` under `dialog.tc-drawer[data-side=...]`.
...
- **Modern browser capabilities**: see `docs/research/modern-browser-capabilities.md` for the comprehensive analysis of native APIs (`<dialog>`, Popover API, `@starting-style`, `<details>`, scroll-snap, View Transitions, `content-visibility`, `:has()`) and the phased migration roadmap. CSS foundation for these APIs is in `templates/app.css`.
...
- **Stylable Select (`appearance: base-select`)**: `SelectProps.Stylable: true` opts into the customizable `<select>` API. Emits `<button><selectedcontent></selectedcontent></button>` inside the select. CSS `.tc-select` in `app.css` styles button, picker (`::picker(select)`), options, arrow (`::picker-icon`), checkmark. Progressive enhancement — non-supporting browsers (Firefox, iOS Safari) ignore the structure and render native `<select>`.
...
- **Global accent-color CSS**: `templates/app.css` sets `accent-color: blue-600` (light) / `blue-400` (dark) on checkboxes, radios, range inputs, and progress bars. Consumers override via `@theme { --color-blue-600: #custom; }`. No Go code changes needed — same theming model as all other components.
