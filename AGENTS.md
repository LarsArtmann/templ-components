...
- 107 generated files across all packages
...
- **Animated Icons:** `icons.AnimatedIcon(name, class)` and `icons.AnimatedIconWithAnimation(name, anim, class)` render any icon with a hover-triggered CSS animation, inspired by [heroicons-animated.com](https://www.heroicons-animated.com/). Pure CSS (zero JavaScript), respects `prefers-reduced-motion`, triggers on both `:hover` and `:focus-within`. 10 animation presets (`AnimPulse`, `AnimBeat`, `AnimBounce`, `AnimWiggle`, `AnimSpin`, `AnimJump`, `AnimNod`, `AnimShake`, `AnimBlink`, `AnimSplit`). Each icon has a sensible default animation via `DefaultAnimation(name)` (Heart→pulse, Bell→wiggle, Settings→spin, Eye→blink, etc.); unmapped icons default to `AnimPulse`; Spinner defaults to `AnimNone` (has its own built-in spin). Per-path animations (`AnimBlink` for Eye, `AnimSplit` for Trash) target `svg path:nth-child(N)` via CSS and require 2-path icons. CSS lives in `templates/custom.css` under `.tc-anim-*` classes. `AnimatedIcon` wraps the SVG in `<span class="tc-anim tc-anim-{type} inline-flex">`; `AnimNone` renders plain `Icon()` without a wrapper.
