...
- **Visual regression goldens expanded.** Added 12 new goldens: Modal (light/dark), Drawer (left-light/right-dark), Input (text-light/dark/error/disabled), Select (light/dark), RTL (button/card). Total: 27 goldens across 6 component types + RTL coverage.
- **Visual coverage metric** (`visualtest/coverage_test.go`). Reports golden-to-component ratio. Currently 27 goldens / 74 components = 36.5%.
