...
- **`art-dupl check` is the canonical duplication gate.** The accepted clone
  ...
  the capture-tools `browser`/`distserver` packages, `headingTag`,
  `chartSeriesGroup`, `calendarMonthNavLink`). See
...
- **`display.KanbanBoard` — dual-transport kanban board.** Horizontally
  scrolling columns of draggable cards with per-card keyboard move buttons
  ("move to previous/next column"), so the board is fully operable without a
  mouse. Both affordances submit one hidden form carrying the move contract
  (`card`, `column`, `index`) — wired through `KanbanBoardProps.Wire`
  (`wire.Action`), it moves cards under htmx (`hx-post`, self-targeting
  `outerHTML` swap) and Datastar (`@post` with form encoding, response-driven
  targeting via `wire.Handler`) from the same markup. The server decodes with
  `display.ParseKanbanMove`. Unwired boards render read-only. ARIA: region
  landmark, labelled columns, polite live-region announcements, card-count
  labels; CSP-safe singleton script; dark-mode + RTL aware; drag feedback CSS
  in `templates/custom.css`. Browser-proven under both runtimes by
...
- **Browser-level wire E2E + visual goldens (visualtest module).** Two chromedp tests drive real Chromium against a page composed from the production pieces — `layout.Base` (self-hosted htmx), `datastar.SDKScript` (pinned bundle served locally, readiness gated on the runtime's `datastar-ready` event), and wired `display.Button`s — clicking both buttons and asserting the fragment lands in `#wire-htmx-out` / `#wire-datastar-out` through `wire.Handler`. A `wire/dual_transport_{light,dark}` screenshot pair pins the section visually (91 visual goldens now).
- **Wired-Button golden snapshots + contract invariants.** Six golden snapshots pin the wired Button render (htmx full/post, datastar, custom event, empty-URL inert, link+wire), and the wire invariant pack pins the cross-dialect guarantees: empty URL → nil for every enum combination, the URL appears in BOTH dialects for every Method/Event/Target combo, Target never renders under Datastar, wire rendering never emits a `<script>` (CSP-safe by construction), plus `Attributes()` benchmarks and a Target-aware `FuzzAction`.
- **`utils/wire` — the transport-agnostic wiring contract: one `wire.Action` spec, both HTMX and Datastar attribute dialects (ADR-0036).** Consumers no longer hand-write dialect-specific attributes per component: `Action{Method, URL, Event, Target}.Attributes()` renders `hx-get`/`hx-target` under htmx (the zero-value Transport default, ADR-0030) or `data-on:click="@get('…')"` under Datastar, composable with every existing component through `BaseProps.Attrs` — `display.Button` additionally takes it directly via a new `Wire` field, so the identical props render under whichever runtime the page loaded. The contract encodes the verified runtime facts rather than guessing: Datastar v1.0.2 fetch actions accept no target option, so `Target` renders only for htmx and Datastar targeting is response-driven — the package exports the `Datastar-Request`/`Datastar-Selector`/`Datastar-Mode`/`HX-Request` header constants that let one HTTP handler serve both transports (the demo's `/api/wire/fragment` endpoint does exactly that, and its test pins the branching). Typed enums ship `TransportIsValid`/`MethodIsValid`/`EventIsValid` with table tests and a fuzz target; `docs/transport-wiring.md` documents the mapping table, the handler recipe, and the consumer-side light-DOM Web Components recipe (library surface unchanged — ADR-0033 stands).
- **Vulnerability gates: a release can no longer ship known-vulnerable dependencies unexamined.** The v1.12.0 push was followed within seconds by 4 HIGH GitHub alerts (`fast-uri`, website npm chain) and nobody had ever asked the question before cutting — so now three layers ask it: `release.sh` step 7 aborts the cut when govulncheck (pinned v1.7.0) finds a symbol-reachable vulnerability in the root module or any sub-module (missing binary fails loud with install instructions); CI's Build & Test runs the same gate on every push; and `scripts/ci-repro.sh --vuln` reproduces both locally, adding `pnpm audit --prod` over `website/` (the layer that actually carried the fast-uri exposure). Verified live: all modules clean, website audit clean after the fast-uri@3.1.7 bump.
- **The release tree assertions are now fixture-tested, and a regression to the SIGPIPE bug class fails CI.** The step-8b assertions that aborted the v1.12.0 cut move into `scripts/lib-release-assertions.sh` (release.sh sources the lib; behavior unchanged), and `scripts/test-release-assertions.sh` builds a throwaway git repo in under a second whose CHANGELOG fixture is 150KB with the heading at the top — the exact pipe geometry where `grep -q` dies of SIGPIPE under pipefail while `grep -c` reads on. Mutation-tested (original 4/4, `grep -q` mutant 2/4) and wired into CI's Lint job plus `ci-repro --lint` so the class is caught on every push, not on the next release attempt. The CI Actionlint step was also pinned to v1.7.12 (was `@latest`, the drift class the golangci-lint pin eliminated) and actionlint joined `nix run .#lint` via nixpkgs (same 1.7.12), closing the local untested-YAML gap (TODO #141's local half).

### Fixed

- **release.sh's vulnerability and tidy gates now survive their first live exercise.** The per-module govulncheck loop ran with `GOWORK=off`, which cannot resolve sibling requires already bumped to the unpushed release version — the gate aborted its first real cut (v1.13.0 attempt one died even earlier, at the icons build: a fresh release worktree lacked the gitignored `go.work` the script's workspace-mode verify phase assumes). govulncheck now runs in workspace mode like the build/test phase above it, and the step-10b tidy sweep no longer aborts the cut: `go mod tidy` ignores go.work entirely and sub-modules carry no replace directives at HEAD, so nothing — root included — can resolve unpushed siblings before the tags propagate. It warns-and-continues instead; the canonical go.sum refresh stays post-propagation, where the v1.12.0 lesson already put it.
- **`display.StatCard` definition-list structure (issue #6).** The label/value pair rendered as a `<dl>` whose group `<div>` wrapped only the `<dd>` while the `<dt>` sat outside the group — invalid `<dl>` content that axe-core flags as a serious `definition-list` violation, forcing every downstream accessibility audit to carry a custom tolerance. The group `<div>` now wraps both the `<dt>` and the `<dd>` (`flex flex-col mt-1` preserves the exact previous stacking: label above, value row below), which is the spec-valid dl grouping and retires the axe finding without any visual change. Structural regression test `TestStatCard_DLGroupContainsDtAndDd` pins the dt+dd-in-one-group contract; goldens updated. Fixes #6.
- **Post-release CI repair: master went red on all three jobs right after the v1.12.0 push.** Build & Test's "Verify no untracked changes" and Lint both traced to the v1.11.0 go.sum-drift class: the post-cut tidy sweep had run before the tags reached the module proxy, so the five published sub-modules' `go.sum` files still held v1.11.0 sibling checksums, and CI's tidy — which can now resolve v1.12.0 — produced a dirty tree plus missing-go.sum-entry lint errors in the icons module. All modules re-tidied against the propagated tags. Visual Regression failed on a separate race: the combined `eyebrow_scrollback_pageheader` capture fired mid-Scrollback-stagger (the entrance animation runs up to ~2.5s, but the settle guard only covered overlay captures with an 800ms element-scoped deadline). `waitAnimationsSettled` now runs on every capture and polls `document.getAnimations()` until every **finite** animation finishes (8s deadline, throttling headroom included) while skipping infinite ones — spinners and shimmer would otherwise burn the whole deadline on every capture; the harness timing test pins all four paths.
...
- **Go toolchain bumped to 1.26.7 across the flake and all 8 modules — closes the GO-2026-5972/6089/6090 exposure.** The demo's `http.Server` reaches `net/http` and `crypto/tls` advisories fixed in 1.26.6, but the locked nixpkgs still packaged 1.26.5 and a wholesale input bump would have drifted `pkgs.templ` (breaking the v0.3.1020 generate pin) and `pkgs.golangci-lint` (breaking lint reproducibility). The flake now pins a dedicated `nixpkgs-go` input (same isolation pattern as `nixpkgs-chromium`), and the `go` directive moves to 1.26.7 in root, utils, icons, errorpage, charts/echarts, datastar, htmx, and visualtest. CI's `setup-go@1.26` resolves the same patch.
- **`scripts/ci-repro.sh` — pre-push reproduction of CI's exact step sequence.** The root-form workspace test run misses every sub-module (the documented `go list ./...` blind spot), so this script mirrors the Build & Test job step-for-step: generate, tracked-file check, tidy (visualtest included), tree verification, vet, build, race tests, per-module GOWORK=off isolation tests, the visualtest compile check (with proxy-flake retry), docs-health drift guard, coverage threshold, and example builds. `--lint`, `--css`, and `--visual` add the other jobs; `--cold` swaps in a throwaway GOCACHE. The coverage threshold check uses awk instead of CI's `bc` so it cannot silently pass on machines without bc.
- **`Upstream watch` workflow for `go-datastar/static` — a new upstream bundle now opens a tracking issue instead of relying on someone noticing.** A weekly scheduled job compares the pinned version in `datastar/go.mod` against the module proxy's `@latest` and opens a single non-duplicating issue (with the re-audit checklist against `docs/datastar-runtime-facts.md`) when they diverge; the bundle-byte contract test stays the hard backstop. Its first exercise arrived immediately: static v0.3.0/v0.4.0 published against the v0.2.0 pin (bumped the next day — see Changed).
- **CI guard: a PR that touches library component code must also diff CHANGELOG.md.** Mechanical enforcement of the always-warm `[Unreleased]` rule (PR #5 shipped component changes with an empty section). Runs via the PR files API — no git history fetch — and fails with the fix in the error message.
- **visualtest sibling-pin policy, enforced instead of tribal.** `scripts/check-version-sync.sh` (pre-commit + CI) now fails if any go.mod — visualtest included — pins a templ-components sibling at a version other than `utils.Version`; `release.sh` step 5b bumps visualtest's pins with everything else, and `visualtest/go.mod` carries local sibling replaces so post-tag re-tidying and release-commit CI resolve locally before the tags reach the proxy (the v1.11.0 nine-day-red class, closed structurally). Policy documented in `docs/modularization/README.md`.
- **`docs/release-checklist.md` — the v1.9.0–v1.11.0 release lessons distilled into one operational page.** What release.sh does, the incident each hardening step exists because of (verify-before-strip, single-EXIT-trap, re-add+tidy, tag lockstep, daemon races), the pre-push audit, and the 24-hour post-push watch.
- **datastar recipe: the server-side additions from the SSE audit.** The `NewResponseFromHTTP` one-liner, `stream.Heartbeat` for proxy keep-alive, a `datastar-fetch` analytics-hook section (`started`/`finished`/`error`/`retrying`/`retries-failed`, no `datastar-sse-error`), and a cross-link to `docs/datastar-runtime-facts.md`.
- **`cmd/tc` scaffolder now ships the datastar package** (LiveRegion, Indicator, SDKScript, SSEErrorHandling) with post-audit patterns by construction, plus the sibling-deps list for `--list-deps`.
- **Visual goldens: 16 new PNGs covering the last untested color surfaces.** StatCard yellow/purple tones, outline success/info buttons (light+dark), the datastar package's first pixel coverage (LiveRegion filled state, Indicator — SDKScript deliberately excluded as pixel-less), and Eyebrow + Scrollback light/dark plus an Eyebrow+PageHeader+Scrollback composition. Same agent-generated caveat as TODO #80 applies.
- **`Master Red Alert` — a red master now opens its own tracking issue instead of sitting unnoticed for days.** A daily scheduled workflow runs `scripts/check-master-green.sh` against the latest CI and Website runs and opens (then keeps commenting on) a single non-duplicating issue while either stays red — the v1.11.0 cut sat red for 9 days because nothing notified on a failing default branch. `check-module-sync.sh` now also covers the visualtest module (8 modules total): its stale sibling requires had been invisible to the guard and only surfaced as a Visual Regression abort.
...
- **Datastar hardening: the bundle contract now pins bytes, not just tokens.** `TestPinnedRuntimeBundleContract` additionally pins the SHA-256 of the embedded `datastar.js` (`4df1f98a…`, byte-identical across static v0.2.0–v0.4.0), closing the gap where a byte-level runtime change passed silently as long as every pinned token survived. A new `TestDatastarVersionConstantNameMatchesValue` guards the literal behind `DatastarVersion1_0_2` so a future `static.Version` move cannot let the constant's name lie about its value. Both failures carry re-audit instructions pointing at `docs/datastar-runtime-facts.md`.
...
- **Daemon regression defused pre-release (2026-09-03): commit 8a9bb87 re-flipped `website/package.json` typescript to ^7.0.2 and prettier-un-minified `examples/demo/static/app.css` (+4951 lines), leaving origin/master red on Build Website (stale lockfile: 6 manifest/lockfile mismatches, `ERR_PNPM_OUTDATED_LOCKFILE` under frozen-lockfile) and CSS Freshness.** Verified first that astro ^7.3.1 still cannot run `astro check` on TS 7 (crashes: the native compiler does not ship the programmatic API — upstream tracks support in withastro/roadmap#1321), then reverted typescript to ^6.0.3, re-synced `pnpm-lock.yaml` (astro ^7.3.1 plus 5 other pending manifest bumps are now actually frozen), and restored the byte-stable minified `app.css` (two consecutive `nix run .#css` compiles produce identical sha256). `astro check` (0 errors, 30 files) and the full website build (13 pages) verified locally.
- **Lint reproducibility: CI now pins golangci-lint to v2.13.2 (was `@latest`, so every upstream release could change lint behavior overnight — exactly how the exhaustruct deprecation drifted in), and the deprecated `exhaustruct` linter is migrated to `exhaustruct_v5`.** The migration initially surfaced 24 findings, all stale `//nolint:exhaustruct` directives that no longer matched the renamed linter; all 51 directives renamed, zero config exclusions added, all modules lint clean.
- **Post-merge convention repair for the StatTone PR: `StatToneIsValid()` was missing** (every closed-set enum must ship `IsValid` + a test entry in the same change — now map-driven off `statTileLookup` so new tones stay in sync). The golden sweep now pins all five tones plus the unknown-value blue fallback and all four outline button variants (10 new goldens), the demo showcases a Stat Card Tones grid and the outline variants alongside the filled buttons, and the FEATURES.md Button/StatCard rows reflect the new surfaces.
- **`datastar.LiveRegion` no longer emits `@get('   ')` for whitespace-only URLs.** A blank-but-not-empty URL passed the non-empty check and would fetch a garbage relative URL (or throw) on every page load; it now degrades to a plain container exactly like the empty case.
- **`writeDatastarPatch` preserves interior blank lines — `<pre>` content survives the SSE round-trip.** Blank lines used to be dropped (silently corrupting preformatted content); they are now emitted as empty-value datalines, which the pinned runtime joins back to newlines (verified against the bundle's parser bytes). CR/LF in selector/mode is replaced with spaces so a crafted selector cannot inject datalines, pinned by `FuzzWriteDatastarPatch` (2.3M execs) plus a blank-line regression test.
- **LiveRegion carries an `aria-busy` loading cue until its first patch.** Auto-started regions render `aria-busy="true"` plus a singleton-guarded, nonced script that clears it on the first `datastar-patch-*` event (or terminal failure), so screen readers get a loading state instead of silence and then announce the streamed content. Manual-start and empty-URL regions render no busy state. Goldens updated; BDD + a11y lenses added for the cue, the clearer script, and the global SSEErrorHandling announcer/toast paths.
- **Demo hardening from the headers-contract audit of every endpoint.** All HTMX fragment endpoints (`/api/items`, `/api/items/123`, `/api/save`, `/api/demo-stats`, `/api/users`) now send `Cache-Control: no-store` so a browser/proxy cache can never serve a stale fragment into a live swap, each contract (consumer hx-target/swap vs response) is pinned by `TestHTMXEndpointHeaders`, and the 2s-ticker SSE test plus the delayed save test are skipped in `-short` mode. The SSE stream also emits `: ping` keep-alive comment frames every 15s, and the SSEErrorHandling demo nonce comes from `demoBaseProps()` instead of a hardcoded string.
- **SDKScript preconnect contract pinned by tests.** CDN loading emits the `<link rel="preconnect">` to the CDN origin (custom CDNs included), self-hosted `Src` omits it (no third-party leak on fully self-hosted deployments), and the CDN+nonce combination is pinned by a golden.
- **Local pnpm root cause identified (`~/.local/bin/node` is a bun 1.3.13 shim that lacks `node:sqlite`); workaround verified.** Prepending a real node to PATH makes `pnpm install --frozen-lockfile` pass in `website/`, which also confirms the lockfile is in sync with the manifest (no remaining splits after the typescript fix). Durable fix (removing the shim) is recorded in TODO_LIST.md.
...
- **`datastar.RetryMode` enum + `LiveRegionProps.Retry` — streams can now self-heal across server restarts.** The pinned runtime's reconnection matrix (verified in the v1.0.2 bundle): a clean stream EOF reconnects **only** under `retry: 'always'`; HTTP ≥ 400 under `'always'`/`'error'`; thrown network errors under every mode; the failure counter resets on every successful connect. The default `'auto'` therefore leaves a LiveRegion permanently stale after a demo/server restart. `RetryAlways` renders `@get(url, {retry: 'always'})`; invalid modes degrade to `auto`; `RetryAuto`/empty keep the bare `@get(url)` form (goldens unchanged). The demo's LiveRegion now uses `RetryAlways`.
- **`datastar.TestPinnedRuntimeBundleContract` — bundle-content guard.** Asserts the embedded `static.Bytes()` contains every token the integration depends on (`datastar-fetch` lifecycle event, `datastar-patch-elements`/`-signals` SSE event names, the retry-mode literals). A `go-datastar/static` pin bump that renames any of them now fails CI instead of silently re-breaking `SSEErrorHandling`, `LiveRegion`, or the demo wire format — the exact failure class that left `SSEErrorHandling` inert since birth.

### Fixed

- **`datastar.LiveRegion` with an empty `URL` rendered `data-init="@get('')"` — the runtime throws `FetchNoUrlProvided` on every page load.** The twin of the Indicator empty-signal bug fixed in this audit (missed then, caught in retrospective): an empty URL now degrades to a plain container with no `data-init`, ready for manual wiring via `data-on:*`.

- **Datastar SSE integration audit: the demo stream never worked, and `SSEErrorHandling` listened for an event the runtime never dispatches.** All findings verified against the pinned Datastar v1.0.2 runtime bundle (`go-datastar/static` v0.2.0). Demo `/api/datastar/stream`: format strings emitted literal `
` text instead of newlines (the browser never dispatched a single event), the event name `datastar-merge-fragments` was removed in Datastar v1.0 (`datastar-patch-elements` + keyed `selector`/`mode`/`elements` datalines is the v1 wire format), the id-less fragment was dropped by the runtime's id-matching default, and the server's `WriteTimeout` killed the stream at 10s (the handler now clears the deadline per-connection via `http.NewResponseController`, streams until client disconnect, and sets `X-Accel-Buffering: no`). Demo `/api/datastar/action`: non-SSE HTML responses are patched via `Datastar-Selector`/`Datastar-Mode` response headers — the endpoint now sends them and its result is actually rendered. `datastar.SSEErrorHandling`: rewritten to listen to the real `datastar-fetch` lifecycle event (`error` with `argsRaw.status`, `retries-failed` after exhausted reconnects; transient `retrying` is console-only to avoid toast spam). `datastar.Indicator`: empty `Signal` rendered `data-show="$"` — the signals object is always truthy, pinning the spinner visible; it now degrades to hidden. `datastar.SDKScript`: now emits `<link rel="preconnect">` to the CDN origin (the `datastarCDNOrigin` helper existed but was never called). Docs (`datastar-integration.md` recipe, package godoc): examples showed `PatchElementsTempl` without `WithSelector` — the runtime throws `PatchElementsExpectedSelector` for non-default modes without one; the recipe also claimed no `'unsafe-eval'` is needed under CSP, but Datastar compiles `data-*` expressions with the `Function()` constructor, which CSP classifies as eval.
