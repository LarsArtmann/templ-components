...
- **Animated icons (heroicons-animated inspired).** `icons.AnimatedIcon(name, class)` and `icons.AnimatedIconWithAnimation(name, anim, class)` render any icon with a hover-triggered CSS animation. 10 animation presets (`AnimPulse`, `AnimBeat`, `AnimBounce`, `AnimWiggle`, `AnimSpin`, `AnimJump`, `AnimNod`, `AnimShake`, `AnimBlink`, `AnimSplit`) covering all heroicons-animated patterns. Pure CSS (zero JavaScript), `prefers-reduced-motion` support, triggers on `:hover` and `:focus-within`. Each icon has a sensible default via `DefaultAnimation()` (Heart→pulse, Bell→wiggle, Settings→spin, Eye→blink, etc.); unmapped icons default to `AnimPulse`. CSS lives in `templates/custom.css` under `.tc-anim-*` classes.
