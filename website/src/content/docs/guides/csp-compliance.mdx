...
The `integration` package contains `TestCSPNonce` which renders every inline-script component and asserts every `<script>` tag has `nonce=`. This prevents CSP regressions.
