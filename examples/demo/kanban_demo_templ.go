...
func kanbanDemo() templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 1, "<p class=\"text-sm text-gray-500 dark:text-gray-400 mb-2\">Drag cards between columns (or use the per-card move buttons revealed on hover/focus — the keyboard path). Every move submits one hidden form with <code>card</code>, <code>column</code>, and <code>index</code>; the server applies the move and re-renders the board. The htmx board swaps via <code>hx-post</code>, the Datastar board via <code>&#64;post</code> with form encoding — same component, same endpoint shape. Each column header also carries an <code>Action</code> add-card slot (a ghost Button wired in the board's dialect) and a <code>Tone</code> status dot. The move form carries a server-issued CSRF token; the bodyless add/reset POSTs enforce same-origin instead, and an add to an unknown column 404s. Each board header has a Reset button that restores the starting layout. Moves apply optimistically (ADR-0041): the card moves the moment you drop it and wears a spinner until the server's re-render lands — the move endpoint deliberately waits 800ms so you can see the pending state. A failed move snaps back with a red flash and a screen-reader alert.</p>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
