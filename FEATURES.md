...
| `SkeletonCardGrid` | FULLY_FUNCTIONAL | Loading card grid       | Responsive grid of N skeleton cards (`SkeletonCardGridProps{Count, ContainerAware}`), single `role="status"`, matches `Grid.ContainerResponsive` when container-aware |
...
- **Responsive:** Mobile-first viewport breakpoints (`sm:`/`md:`/`lg:`) plus opt-in **container queries** — 8 components accept `ContainerAware` (or `Grid.ContainerResponsive`) to adapt to their parent container width via `@container` instead of the viewport (ADR-0018). **Fluid typography** via container query units (`cqi`) — `.tc-fluid-*` utility classes scale text with container width (ADR-0018 analog; recipe: `docs/recipes/fluid-typography.md`).
