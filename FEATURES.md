...
- **Test Coverage:** 74% average across packages. BDD + golden-file HTML snapshots + a11y + benchmark + integration tests, plus pixel-level **visual regression** tests (`visualtest/` — chromedp + pixelmatch, `nix run .#visual`) and drift-guard tests enforcing doc/code count consistency.
