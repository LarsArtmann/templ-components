...
| `AppShell`    | FULLY_FUNCTIONAL | Admin dashboard shell      | Sidebar + sticky header + main content, `lg:grid lg:grid-cols-[var(--tc-sidebar-w)_minmax(0,1fr)] min-h-dvh`, `SidebarWidth` enum (SM/MD/LG/Auto), optional `MobileNav` slot, renders inside `Base`'s `<main>` (no nested `<main>`) |
