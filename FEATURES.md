...
- **Test Coverage:** 72.3% total statement coverage across library packages (range 48.2%–81.8%; CI-enforced ≥ 70% via `go test -race -coverprofile=...`; recompute with `nix run .#coverage`). BDD + golden-file HTML snapshots (`utils/golden` in 5 packages, 252 baselines) + a11y + benchmark + integration tests, plus pixel-level **visual regression** tests (`visualtest/` — chromedp + pixelmatch, `nix run .#visual`; 159 goldens incl. charts, dark-mode variants, and RTL coverage) and drift-guard tests enforcing doc/code count consistency (component, enum, generated-file, dark-mode, motion-reduce, RTL, container-query, CSS-freshness, lint-config).
