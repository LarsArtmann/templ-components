...
| #   | Task                                        | Blocker                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 110 | Fix broken `v1.8.0` git tag                 | The annotated tag `v1.8.0` points to commit `685bee8`, where `utils.Version` is still `1.7.0` and `FEATURES.md` says `1.7.0` (the release bypassed `scripts/release.sh` — a CHANGELOG-only commit was tagged). The version was bumped to `1.8.0` in the later commit `c11d2e4`. Consumers who `go get @v1.8.0` get a lying `utils.Version`. **Needs a user decision:** force-move the tag (requires `--force-with-lease` + approval) OR cut a corrective patch/minor release via `scripts/release.sh`. |
...
_All actionable items have been completed or deferred. See Blocked and Deferred sections below._
