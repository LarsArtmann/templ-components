...
declare -A MODULE_PATHS=(
	...
	["visualtest"]="github.com/larsartmann/templ-components/visualtest"
	["website"]="github.com/larsartmann/templ-components/website"
)
...
done < <(grep -rn 'replace.*=>' go.mod utils/go.mod icons/go.mod errorpage/go.mod charts/echarts/go.mod datastar/go.mod htmx/go.mod visualtest/go.mod website/go.mod 2>/dev/null || true)
...
if [ -n "$SHARED_VERSION" ]; then
	for modfile in go.mod icons/go.mod errorpage/go.mod charts/echarts/go.mod datastar/go.mod htmx/go.mod visualtest/go.mod website/go.mod; do
...
echo "Module sync check: OK (9 modules incl. visualtest + website, all paths and versions consistent)."
